...
const (
	a6PriorCaskBlob     = "ceed648bfece4555e8310b6e894fedc847520960"
	a6FrozenFormulaBlob = "4ebc6c38925002dec00759823d4dd847a499818a"
)
...
func TestHomebrewFormulaBridge_A6UpdatesOnlyCaskThenIsIdempotent(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("publish A6 Cask: %v\n%s", err, output)
	}
	if cask := fixture.apiContent(t, "cask.json"); !strings.Contains(cask, `version "0.50.76"`) {
		t.Fatalf("published Cask is not v0.50.76:\n%s", cask)
	}
...
func newHomebrewBridgeFixture(t *testing.T) homebrewBridgeFixture {
	...
	fixture.writeAPIContent(t, "cask.json", a6PriorCaskBlob, fixture.cask)
	fixture.writeAPIContent(t, "formula.json", a6FrozenFormulaBlob, homebrewBridgeFormula(t))
...
func (fixture homebrewBridgeFixture) run(overrides map[string]string) ([]byte, error) {
	environment := map[string]string{
		...
		"GITHUB_REF_NAME": "v0.50.76", "COMPANION_VERSION": "0.50.76",
...
func homebrewBridgeChecksums() string {
	...
	for index, name := range names {
		fmt.Fprintf(&output, "%s  autopus-adk_0.50.76_%s.tar.gz\n", bridgeDigests[index], name)
	}
	fmt.Fprintf(&output, "%s  autopus-adk_0.50.76_windows_amd64.zip\n", strings.Repeat("5", 64))
