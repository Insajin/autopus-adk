...
func TestHomebrewFormulaBridge_A22UpdatesOnlyCaskThenIsIdempotent(t *testing.T) {
	...
	if cask := fixture.apiContent(t, "cask.json"); !strings.Contains(cask, `version "0.50.94"`) {
		t.Fatalf("published Cask is not v0.50.94:\n%s", cask)
	}
...
func homebrewBridgeChecksums() string {
	...
	for index, name := range names {
		fmt.Fprintf(&output, "%s  autopus-adk_0.50.94_%s.tar.gz\n", bridgeDigests[index], name)
	}
	fmt.Fprintf(&output, "%s  autopus-adk_0.50.94_windows_amd64.zip\n", strings.Repeat("5", 64))
