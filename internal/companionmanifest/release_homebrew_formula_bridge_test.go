...
const (
	a14PriorTapCommit    = "d8dc4c78f42a7c5e30176334b607b036be3bd677"
	a14PriorCaskBlob     = "524ade82d6466da8ad6d5c173e0b4a214fdbc21f"
	a14FrozenFormulaBlob = "4ebc6c38925002dec00759823d4dd847a499818a"
)
...
func TestHomebrewFormulaBridge_A14UpdatesOnlyCaskThenIsIdempotent(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("publish A14 Cask: %v\n%s", err, output)
	}
	if cask := fixture.apiContent(t, "cask.json"); !strings.Contains(cask, `version "0.50.85"`) {
		t.Fatalf("published Cask is not v0.50.85:\n%s", cask)
	}
...
func newHomebrewBridgeFixture(t *testing.T) homebrewBridgeFixture {
	...
	fixture.writeAPIContent(t, "cask.json", a14PriorCaskBlob, fixture.cask)
	fixture.writeAPIContent(t, "formula.json", a14FrozenFormulaBlob, homebrewBridgeFormula(t))
	branch := `{"ref":"refs/heads/main","object":{"type":"commit","sha":"` +
		a14PriorTapCommit + `","url":"https://example.invalid/prior-commit"}}`
...
func homebrewBridgeChecksums() string {
	...
	for index, name := range names {
		fmt.Fprintf(&output, "%s  autopus-adk_0.50.85_%s.tar.gz\n", bridgeDigests[index], name)
	}
	fmt.Fprintf(&output, "%s  autopus-adk_0.50.85_windows_amd64.zip\n", strings.Repeat("5", 64))
