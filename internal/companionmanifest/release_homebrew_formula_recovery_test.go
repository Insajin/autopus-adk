...
func TestFormulaRecoveryWorkflow_ManualExactA28LeastPrivilege(t *testing.T) {
	...
	for _, version := range regexp.MustCompile(`v?[0-9]+\.[0-9]+\.[0-9]+`).FindAllString(raw, -1) {
		if version != "v0.50.117" && version != "v4.1.2" && version != "v3.1.2" {
			t.Fatalf("recovery workflow references non-A28 version %q", version)
		}
	}
	if strings.Count(raw, "v0.50.117") != 2 {
		t.Fatalf("recovery workflow must name v0.50.117 only in invocation guidance and exact job guard")
	}
}
...
func TestFormulaRecoveryWorkflow_PinsActionsSourceAndR2Tag(t *testing.T) {
	...
	if strings.Contains(raw, "ADK_KEY_ROTATION_VERIFIED") || strings.Contains(raw, "materialize-key-rotation-authority") {
		t.Fatal("normal A28 recovery retains the A22 bridge rotation gate")
	}
}
...
func TestFormulaRecoveryWorkflow_RequiresExactSealedTagAuthorityBeforeTapToken(t *testing.T) {
	...
	for _, required := range []string{
		`readonly release_ref='refs/tags/v0.50.117'`, `--sealed-runtime) mode=sealed-runtime`,
...
func TestFormulaRecoveryWorkflow_SharedVerifierOwnsExact15AssetK3Proof(t *testing.T) {
	...
	for _, required := range []string{
		...
		"exactly fifteen A28 normal release assets",
	} {
		if !strings.Contains(helper, required) {
			t.Fatalf("shared A28 release verifier missing %q", required)
		}
	}
	for _, platform := range []string{"darwin_amd64", "darwin_arm64", "linux_amd64", "linux_arm64"} {
		if !strings.Contains(helper, "autopus-adk_${RELEASE_VERSION}_"+platform+".tar.gz") {
			t.Fatalf("shared A28 verifier missing archive %s", platform)
		}
	}
	for _, arch := range []string{"amd64", "arm64"} {
		for _, extension := range []string{"tar.gz", "zip"} {
			if !strings.Contains(helper, "autopus-adk_${RELEASE_VERSION}_windows_"+arch+"."+extension) {
				t.Fatalf("shared A28 verifier missing windows %s.%s", arch, extension)
			}
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(helper, forbidden) {
			t.Fatalf("shared A28 verifier retains forbidden bridge/external authority %q", forbidden)
		}
	}
}
