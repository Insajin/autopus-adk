...
func TestFormulaRecoveryWorkflow_ManualExactA18LeastPrivilege(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		"${{ inputs.", "github.event.inputs", "refs/tags/v*", "v0.50.69", "v0.50.70", "v0.50.71", "v0.50.72", "v0.50.73", "v0.50.74", "v0.50.75", "v0.50.76", "v0.50.77", "v0.50.78", "v0.50.79", "v0.50.80", "v0.50.81", "v0.50.82", "v0.50.83", "v0.50.84", "v0.50.85", "v0.50.86", "v0.50.87", "v0.50.88",
	} {
	...
	for _, version := range regexp.MustCompile(`v?[0-9]+\.[0-9]+\.[0-9]+`).FindAllString(raw, -1) {
		if version != "v0.50.89" && version != "0.50.89" &&
			version != "v4.1.2" && version != "v3.1.2" {
			t.Fatalf("recovery workflow references non-A18 version %q", version)
		}
	}
...
func TestFormulaRecoveryWorkflow_PinsCheckoutAndTapAppScope(t *testing.T) {
	...
	for _, exact := range []string{
		"ref: refs/tags/v0.50.89", "fetch-depth: 0", "persist-credentials: false",
		"client-id: ${{ vars.HOMEBREW_APP_CLIENT_ID }}",
		"private-key: ${{ secrets.HOMEBREW_APP_PRIVATE_KEY }}",
...
func TestFormulaRecoveryWorkflow_ValidatesSourceAndImmutableReleaseEvidence(t *testing.T) {
	...
	for _, required := range []string{
		"git rev-parse --verify 'HEAD^{commit}'", "mktemp", "GITHUB_REF_NAME='v0.50.89'",
		"autopus-v0.50.89-checksums.txt",
...
func TestFormulaRecoveryWorkflow_RunsOnlyIdempotentA18CaskWithAllowlistedEnvironment(t *testing.T) {
