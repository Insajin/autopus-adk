...
func TestReleaseWorkflow_ExactA29ProtectedNormalLane(t *testing.T) {
	release := readReleaseFile(t, ".github/workflows/release.yaml")
	for _, required := range []string{
		"- 'v0.50.118'", "if: github.ref == 'refs/tags/v0.50.118'",
		"needs: [ci, security, omp-production-evidence]", "adk-companion-release",
		"Validate exact R2-signed A29 source", "COMPANION_RELEASE_TAG_SIGNATURE_REQUIRED=1",
		...
		"name: omp-context-evidence-v0.50.118", "--mode active",
		"autopus.adk_release_reservation.v1", "release_tag:\"v0.50.118\"",
		...
		if !strings.Contains(release, required) {
			t.Fatalf("A29 release workflow missing %q", required)
		}
	}
	...
	for _, forbidden := range []string{
		"v0.50.117", "omp-canonical-bridge-candidate", "omp-context-bridge-release.v1.json",
		"adk-key-rotation-v1.json", "adk-key-rotation-v1.sig", "--expected-signing-key-id",
		"releases/assets/${asset_id}", "--armed", "- 'v*'", "- v*",
	} {
		if strings.Contains(release, forbidden) {
			t.Fatalf("A29 normal release workflow retains forbidden contract %q", forbidden)
		}
	}
}
