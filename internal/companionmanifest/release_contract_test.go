...
func TestReleaseWorkflow_ExactA23ProtectedNormalLane(t *testing.T) {
	...
	for _, required := range []string{
		...
		if !strings.Contains(release, required) {
			t.Fatalf("A23 release workflow missing %q", required)
		}
	}
	...
	for _, forbidden := range []string{
		"v0.50.109", "omp-canonical-bridge-candidate", "omp-context-bridge-release.v1.json",
		"adk-key-rotation-v1.json", "adk-key-rotation-v1.sig", "--expected-signing-key-id",
		"releases/assets/${asset_id}", "--armed", "- 'v*'", "- v*",
	} {
		if strings.Contains(release, forbidden) {
			t.Fatalf("A23 normal release workflow retains forbidden contract %q", forbidden)
		}
	}
}
