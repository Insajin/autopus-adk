...
func TestReleaseWorkflow_ExactA27ProtectedNormalLane(t *testing.T) {
	release := readReleaseFile(t, ".github/workflows/release.yaml")
	for _, required := range []string{
		"- 'v0.50.116'", "if: github.ref == 'refs/tags/v0.50.116'",
		"needs: [ci, security, omp-production-evidence]", "adk-companion-release",
		"Validate exact R2-signed A27 source", "COMPANION_RELEASE_TAG_SIGNATURE_REQUIRED=1",
		...
		"name: omp-context-evidence-v0.50.116", "--mode active",
		"autopus.adk_release_reservation.v1", "release_tag:\"v0.50.116\"",
		...
		if !strings.Contains(release, required) {
			t.Fatalf("A27 release workflow missing %q", required)
		}
	}
	...
	for _, forbidden := range []string{
		"v0.50.115", "omp-canonical-bridge-candidate", "omp-context-bridge-release.v1.json",
		"adk-key-rotation-v1.json", "adk-key-rotation-v1.sig", "--expected-signing-key-id",
		"releases/assets/${asset_id}", "--armed", "- 'v*'", "- v*",
	} {
		if strings.Contains(release, forbidden) {
			t.Fatalf("A27 normal release workflow retains forbidden contract %q", forbidden)
		}
	}
}
...
func TestReleaseWorkflow_ExactFifteenAssetSet(t *testing.T) {
	release := readReleaseFile(t, ".github/workflows/release.yaml")
	assets := []string{
		"autopus-adk_0.50.116_darwin_amd64.tar.gz",
		"autopus-adk_0.50.116_darwin_arm64.tar.gz",
		"autopus-adk_0.50.116_linux_amd64.tar.gz",
		"autopus-adk_0.50.116_linux_arm64.tar.gz",
		"autopus-adk_0.50.116_windows_amd64.tar.gz",
		"autopus-adk_0.50.116_windows_amd64.zip",
		"autopus-adk_0.50.116_windows_arm64.tar.gz",
		"autopus-adk_0.50.116_windows_arm64.zip",
