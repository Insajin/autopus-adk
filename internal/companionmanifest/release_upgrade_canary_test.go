...
func TestUpgradeCanary_ManualExactPublicSignedA27ToA28(t *testing.T) {
	...
	admission := upgradeCanaryStepNamed(t, workflow, "Admit exact public signed A27 to A28 upgrade")
	for _, required := range []string{
		"posix-upgrade-canary.sh live 0.50.116 0.50.117",
		`.previous_public_version == "0.50.116"`,
		`.candidate_public_version == "0.50.117"`,
...
func TestUpgradeCanary_BindsAvailablePublicPredecessorAndCandidateProvenance(t *testing.T) {
	...
	for _, required := range []string{
		"releases/383500138",
		"39101f97302267d052ed18b8aafa9ec23278c091",
		"fbe502c05f84d5eeb81b089b2344c47329ab4543",
		"95ba04d8499b00af86f6794e38a4da1b6d497a6d",
		"sha256:f3c7b2d148b370a7d200a3474ceb342f933c3adead08eca0044f63fbc8ed30bf",
		"releases/tags/v0.50.117", "git/ref/tags/v0.50.116",
...
func TestUpgradeCanary_PinsActionsAndUploadsDiagnosticReceipt(t *testing.T) {
	...
	if upload.Uses != "actions/upload-artifact@ea165f8d65b6e75b540449e92b4886f43607fa02" ||
		upload.With["name"] != "upgrade-canary-0.50.116-to-0.50.117" || upload.With["retention-days"] != 30 {
...
func TestUpgradeCanary_ModelsPublicV109WithoutManagedOMPConfig(t *testing.T) {
	...
	if !strings.Contains(raw, absence) {
		t.Fatalf("upgrade canary does not verify the v0.50.116 OMP config absence")
	}
	...
	if strings.Contains(raw, `cat "$PROJECT/.omp/config.yml"`) {
		t.Fatal("upgrade canary still reads the absent v0.50.116 OMP config")
	}
	if !strings.Contains(
		raw,
