...
func TestReleaseWorkflow_A27PublishesFreshPolicyOwnedK3Evidence(t *testing.T) {
	...
	for _, required := range []string{
		...
		if !strings.Contains(combined, required) {
			t.Fatalf("A27 normal evidence contract missing %q", required)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(combined, forbidden) {
			t.Fatalf("A27 normal evidence contract contains bridge authority %q", forbidden)
		}
	}
	for _, required := range []string{
		...
		if !strings.Contains(config, required) {
			t.Fatalf("GoReleaser A27 evidence asset missing %q", required)
		}
	}
	for _, asset := range []string{
		...
		if !strings.Contains(current, asset) {
			t.Fatalf("current A27 verifier missing exact asset %q", asset)
		}
	}
}
...
func TestReleaseSourceValidator_A22RotationSidecarRemainsHistoricalOnly(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(release, forbidden) {
			t.Fatalf("A22 bridge authority escaped into active A27 workflow: %q", forbidden)
		}
	}
}
