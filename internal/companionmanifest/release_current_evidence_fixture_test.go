...
func newCurrentReleaseFixture(t *testing.T) *currentReleaseFixture {
	...
	fixture := &currentReleaseFixture{
		...
		release: currentReleaseDocument{ID: 410116, TagName: "v0.50.116",
...
func (f *currentReleaseFixture) run() (string, error) {
	...
	command.Env = []string{
		...
		"GH_TOKEN=fixture-fallback-token", "COMPANION_RELEASE_ID=410116",
...
case "$endpoint" in
  repos/Insajin/autopus-adk/releases/tags/v0.50.116) exec cat "$MOCK_CURRENT_RELEASE_STATE/release.json" ;;
