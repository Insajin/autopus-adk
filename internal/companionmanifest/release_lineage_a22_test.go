...
func TestReleaseSourceValidator_A22AcceptsAnnotatedA21DescendantAndExactPins(t *testing.T) {
	...
	runGit(t, dir, "tag", "-am", "A22 release candidate", "v0.50.99")
	output, err := runReleaseSourceValidator(t, dir, "v0.50.99", sha,
...
func TestReleaseSourceValidator_A22RejectsUnsignedTagWhenProductionSignatureIsRequired(t *testing.T) {
	...
	sha := strings.TrimSpace(runGit(t, dir, "rev-parse", "HEAD"))
	tree := strings.TrimSpace(runGit(t, dir, "rev-parse", "HEAD^{tree}"))
	runGit(t, dir, "tag", "-am", "unsigned A22 release candidate", "v0.50.99")
	output, err := runReleaseSourceValidator(t, dir, "v0.50.99", sha,
...
func TestLineageVerifier_A22UsesExactA21ReleaseCoordinate(t *testing.T) {
	...
	for _, required := range []string{
		"A21_REPOSITORY='Insajin/autopus-adk' A22_TAG='v0.50.99' A22_VERSION='0.50.99'",
