...
const releaseLineagePinsScript = "scripts/companion-release/verify-public-key-lineage-pins.sh"

// 각 좌표가 verify-public-key-lineage-pins.sh에서 정확히 한 번씩 보아야 하는 직전 릴리즈 증거 핀이다.
// 값은 게시된 immutable 릴리즈에서 관측한 것이므로 좌표별 데이터이고, 여기 한 항목이 전부다.
var releasePhasePriorPins = map[string][]string{
	"A8": {
...
func TestLineageVerifier_PhasesPinExactImmutablePredecessorEvidence(t *testing.T) {
	pins := readReleaseFile(t, releaseLineagePinsScript)
	for _, phase := range releasePhases {
		declarations, pinned := releasePhasePriorPins[phase.phase]
		if !pinned {
			continue
		}
		t.Run(phase.phase, func(t *testing.T) {
			for _, declaration := range declarations {
				if strings.Count(pins, declaration) != 1 {
					t.Fatalf("%s prior evidence pin drifted: %s", phase.phase, declaration)
				}
			}
			if !phase.forbidsHistoricalLinuxPins {
				return
			}
			for historical := 0; historical <= 13; historical++ {
				if strings.Contains(pins, fmt.Sprintf("A%d_LINUX_", historical)) {
					t.Fatalf("historical A%d Linux pin must remain absent", historical)
				}
			}
		})
	}
	// 모든 좌표가 표에 있어야 한다 — 새 행이 기대값 없이 들어오면 여기서 드러난다.
	declared := make(map[string]struct{}, len(releasePhases))
	for _, phase := range releasePhases {
		declared[phase.phase] = struct{}{}
	}
	for phase := range releasePhasePriorPins {
		if _, known := declared[phase]; !known {
			t.Fatalf("prior pins declared for unknown phase %q", phase)
		}
	}
}
