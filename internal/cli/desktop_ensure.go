...
func newDesktopEnsureCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Checks desktop runtime bootstrap state and brings it to ready without starting the legacy local-host daemon.\n" +
			...
			"For installed desktop operation, prefer the desktop app readiness flow or `autopus-desktop-runtime desktop ensure`; this ADK path is retained as a migration shim for compatibility and harness verification.",
