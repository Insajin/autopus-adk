...
func TestWorkflowContextProductOverlay_ActiveReuseAndShadowRollbackStayTaskOwned(t *testing.T) {
	...
	body := string(activeBody)
	assert.Contains(t, body, "methodOrder: [snapcompact]")
	assert.Contains(t, body, "enablePiProject: true")
	assert.Contains(t, body, "customDirectories: []")
	assert.NotContains(t, body, "strategy:")
	assert.NotContains(t, body, "remoteEnabled")
	assert.NotContains(t, body, ".agents/skills")
