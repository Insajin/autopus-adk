...
const (
	headlessEventSchemaVersion = "1.0.0"
	headlessEventCommand       = "auto connect --headless"
)
...
func EmitEvent(ev HeadlessEvent) {
	if ev.SchemaVersion == "" {
		ev.SchemaVersion = headlessEventSchemaVersion
	}
	if ev.Command == "" {
		ev.Command = headlessEventCommand
	}
	if ev.GeneratedAt.IsZero() {
		ev.GeneratedAt = time.Now().UTC()
	}
