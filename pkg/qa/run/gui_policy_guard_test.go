...
`)
	...
	graph := readFinalArtifact(t, projectPath(dir, result.ManifestPaths[0]), manifest, "journey_graph")
...
`)
	...
	assert.NotContains(t, string(mustReadFile(t, projectPath(dir, result.ManifestPaths[0]))), "raw-screenshot.png")
}
