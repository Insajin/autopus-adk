...
func WriteFile(report Report, path string) error {
	...
	tmpName := tmp.Name()
	// The cleanup errors are discarded on purpose and marked so: each of these
	// paths already has a more important error to return, and a failed unlink of
	// a temp file must not replace it.
