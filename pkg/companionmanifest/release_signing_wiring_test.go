...
func TestReleaseSigning_WorkflowMaterializesAndPreflightsKeyBeforeGoReleaser(t *testing.T) {
	...
	for _, required := range []string{
		"v0.50.84", "autopus-v0.50.84-checksums.txt",
		"GITHUB_REF_NAME='v0.50.84'", "COMPANION_VERSION='0.50.84'",
	} {
		if !strings.Contains(workflowSource, required) {
			t.Fatalf("v0.50.84 signing workflow missing exact version contract %q", required)
		}
	}
	if strings.Contains(workflowSource, "v0.50.83") {
		t.Fatal("v0.50.84 signing workflow still exposes the frozen A12 coordinate")
	}
	...
	if strings.Contains(workflowSource, "release-k2") {
		t.Fatal("v0.50.84 workflow must keep K2 offline and sign with K1 only")
	}
}
