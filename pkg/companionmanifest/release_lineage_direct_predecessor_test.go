...
func directPredecessorPinReplacements(fixture *executableLineageFixture) map[string]string {
	...
	case publicKeyReceiptA15Tag:
		prefix = "A14"
	case publicKeyReceiptA16Tag:
		prefix = "A15"
	case publicKeyReceiptA17Tag:
		prefix = "A16"
	case publicKeyReceiptA18Tag:
		prefix = "A17"
	case publicKeyReceiptA19Tag:
		prefix = "A18"
	case publicKeyReceiptA20Tag:
		prefix = "A19"
	case publicKeyReceiptA21Tag:
		prefix = "A20"
	case publicKeyReceiptA22Tag:
		prefix = "A21"
	default:
		return nil
	}
	...
	if prefix == "A17" || prefix == "A18" || prefix == "A19" || prefix == "A20" || prefix == "A21" {
		replacements[prefix+"_RELEASE_ID"] = fixture.pins.releaseID
	}
	if prefix == "A14" || prefix == "A15" || prefix == "A16" || prefix == "A17" || prefix == "A18" || prefix == "A19" || prefix == "A20" || prefix == "A21" {
	...
	if prefix == "A7" || prefix == "A8" || prefix == "A9" || prefix == "A10" || prefix == "A11" || prefix == "A12" || prefix == "A13" || prefix == "A14" || prefix == "A15" || prefix == "A16" || prefix == "A17" || prefix == "A18" || prefix == "A19" || prefix == "A20" || prefix == "A21" {
...
func TestDirectPredecessorPinReplacements_LinuxPinsBeginAtA14(t *testing.T) {
	...
	replacements = directPredecessorPinReplacements(&executableLineageFixture{
		currentTag: publicKeyReceiptA16Tag, pins: pins,
	})
	if replacements["A15_LINUX_AMD64_ARCHIVE_SHA256"] != "linux-amd64" ||
		replacements["A15_LINUX_ARM64_ARCHIVE_SHA256"] != "linux-arm64" {
		t.Fatalf("A16 direct predecessor Linux replacements = %#v", replacements)
	}
	replacements = directPredecessorPinReplacements(&executableLineageFixture{
		currentTag: publicKeyReceiptA17Tag, pins: pins,
	})
	if replacements["A16_LINUX_AMD64_ARCHIVE_SHA256"] != "linux-amd64" ||
		replacements["A16_LINUX_ARM64_ARCHIVE_SHA256"] != "linux-arm64" {
		t.Fatalf("A17 direct predecessor Linux replacements = %#v", replacements)
	}
	replacements = directPredecessorPinReplacements(&executableLineageFixture{
		currentTag: publicKeyReceiptA18Tag, pins: pins,
	})
	...
	replacements = directPredecessorPinReplacements(&executableLineageFixture{
		currentTag: publicKeyReceiptA20Tag, pins: pins,
	})
	if replacements["A19_LINUX_AMD64_ARCHIVE_SHA256"] != "linux-amd64" ||
		replacements["A19_LINUX_ARM64_ARCHIVE_SHA256"] != "linux-arm64" {
		t.Fatalf("A20 direct predecessor Linux replacements = %#v", replacements)
	}
	replacements = directPredecessorPinReplacements(&executableLineageFixture{
		currentTag: publicKeyReceiptA21Tag, pins: pins,
	})
	if replacements["A20_LINUX_AMD64_ARCHIVE_SHA256"] != "linux-amd64" ||
		replacements["A20_LINUX_ARM64_ARCHIVE_SHA256"] != "linux-arm64" {
		t.Fatalf("A21 direct predecessor Linux replacements = %#v", replacements)
	}
	replacements = directPredecessorPinReplacements(&executableLineageFixture{
		currentTag: publicKeyReceiptA22Tag, pins: pins,
	})
	if replacements["A21_LINUX_AMD64_ARCHIVE_SHA256"] != "linux-amd64" ||
		replacements["A21_LINUX_ARM64_ARCHIVE_SHA256"] != "linux-arm64" {
		t.Fatalf("A22 direct predecessor Linux replacements = %#v", replacements)
	}
}
