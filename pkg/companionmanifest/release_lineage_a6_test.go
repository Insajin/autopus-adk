...
const (
	publicKeyReceiptA6Tag     = "v0.50.77"
	publicKeyReceiptA6Version = "0.50.77"
)
...
func TestReleasePublicKeyReceipt_GoReleaserA6FixtureProducesCurrentArtifacts(t *testing.T) {
	...
	for _, architecture := range []string{"amd64", "arm64"} {
		...
		if !bytes.Contains(manifest, []byte(`"version":"0.50.77"`)) {
...
func exactA6TagVersionGuard(source string) bool {
	return exactLineageTagVersionGuard(source, "77")
}
