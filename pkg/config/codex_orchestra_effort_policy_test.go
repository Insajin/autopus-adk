...
func TestCodexOrchestraEffortPolicy(t *testing.T) {
	...
	if got, want := quality.CodexOrchestraProfile(), (CodexProfile{Model: CodexAstraModel, Effort: CodexEffortMax}); got != want {
		t.Errorf("CodexOrchestraProfile() = %#v, want %#v", got, want)
	}

	if got, want := quality.CodexSupervisorProfile(), (CodexProfile{Model: CodexAstraModel, Effort: CodexEffortUltra}); got != want {
