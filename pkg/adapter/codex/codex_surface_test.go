...
func TestCodexAdapter_Generate_WorkflowSurfacesUseCodexConventions(t *testing.T) {
	...
	for _, path := range []string{
		...
		assert.Contains(t, content, "request_user_input", path)
		assert.Contains(t, content, "tool/requestUserInput", path)
	...
	assert.Contains(t, string(autoIdeaSkill), "request_user_input")
	assert.Contains(t, string(autoIdeaSkill), "tool/requestUserInput")
	...
	assert.Contains(t, string(autoIdeaSkill), "Codex native worker fallback으로 진행할까요?")
	assert.Contains(t, string(autoIdeaSkill), "orchestra_unavailable")
	assert.NotContains(t, string(autoIdeaSkill), "Sequential Thinking으로 fallback")
	...
	assert.Contains(t, string(autoPlanSkill), "request_user_input")
	assert.Contains(t, string(autoPlanSkill), "tool/requestUserInput")
