...
func preserveUserCodexModelSettings(rendered, existing string) string {
	...
	generated := isAutopusGeneratedCodexConfig(existing)
	var section string
	...
	for i, line := range lines {
		...
		key, renderedValue, ok := parseCodexConfigAssignment(trimmed)
		if !ok || !isUserOwnedCodexConfigKey(section, key) {
			continue
		}
		if replacement, ok := overrides[section+"."+key]; ok {
			if shouldUseRenderedCodexEffortDefault(generated, section, key, renderedValue, replacement) {
				continue
			}
...
func isAutopusGeneratedCodexConfig(content string) bool {
	return strings.Contains(content, "# Codex configuration (auto-generated by Autopus-ADK)")
}

func shouldUseRenderedCodexEffortDefault(generated bool, section, key, renderedValue, existingValue string) bool {
	if !generated || section != "" || key != "model_reasoning_effort" {
		return false
	}
	return existingValue == `"medium"` && renderedValue != existingValue
}
