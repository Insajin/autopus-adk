...
func TestRender_MultiRepoSections(t *testing.T) {
	...
	assert.Contains(t, ds.Architecture, "### QA Target Resolution")
	assert.Contains(t, ds.Architecture, "auto qa init --project-dir autopus-desktop --auto --loop")
	assert.Contains(t, ds.Architecture, "Root `.autopus/qa/**` is generated/runtime evidence")
