...
- 새 migration pair/stem 번호는 해당 directory 안의 기존 `*.sql` 파일만 기준으로 `max(existing_number)+1`을 사용합니다. 다른 프로젝트나 workspace root의 번호를 섞지 않습니다.
...
- `.up.sql`과 `.down.sql`은 같은 번호와 같은 stem (same stem)을 사용합니다. 예: `000425_add_table.up.sql` ↔ `000425_add_table.down.sql`.
