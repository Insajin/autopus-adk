...
- **릴리스 아티팩트 게시자 서명 Stage 1 준비 (SPEC-ADK-RELEASE-SIGNING-001, 진행 중)** (2026-07-17): v0.50.73부터 `checksums.txt.signatures` V1 envelope를 게시할 수 있도록 ECDSA P-256 다중 서명 producer와 Go self-update verifier를 추가했다. envelope는 4 KiB·16레코드 상한, exact LF/header, full lowercase SPKI SHA-256 fingerprint, canonical base64/DER, duplicate 거부를 적용하며, 모든 레코드를 파싱한 뒤 known active key의 서명 중 하나 이상이 통과해야 `checksums.txt`를 신뢰한다. 기존 4인자 `DownloadAndVerify` API는 유지하고 안전한 sibling URL 파생에 실패하면 checksum-only로 돌아가지 않는다. release workflow는 raw private-key secret을 짧은 step에서 0600 파일로 만든 뒤 checked-in active K1 public key·full fingerprint와 exact pair인지 확인하고, GoReleaser에는 파일 경로만 전달한다. producer는 키 파일을 한 개 이상 받아 fingerprint 순으로 레코드를 만들며 기존 cosign `checksums.txt.bundle`도 유지한다. 실제 GoReleaser v2.17.0 synthetic-key oracle에서 envelope 생성과 Go 검증을 확인했다. 새 K1과 offline-next K2의 encrypted local custody·recovery를 검증하고 두 public pin을 Go consumer와 checked-in 정본에 함께 선배포했다. v0.50.73 workflow는 K1만 사용하며 K2는 GitHub secret과 signing input에 넣지 않는다. 별도의 off-device 독립 매체 보관은 현 환경에서 확인하지 못했으므로 완료했다고 주장하지 않는다. **아직 완료가 아니다**: K1 GitHub Environment secret·exact pair 확인과 v0.50.73 live asset 검증이 남아 있으며, 그 증거가 나오기 전에는 `install.sh`와 `install.ps1`을 fail-closed로 전환하지 않는다. 두 installer는 Stage 2 대상이다. 또한 현재 공식 installer가 raw GitHub main에서 실행되므로 repository main과 release assets를 함께 장악한 공격까지 방어한다고 주장하지 않는다.
