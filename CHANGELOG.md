...
- **v0.50.79 companion A8 핫픽스 릴리스 준비(PENDING)** (2026-07-20): 릴리스 진입점을 새 annotated tag `v0.50.79`로만 제한하고, 이미 게시된 immutable `v0.50.78`은 이동하거나 재사용하지 않는다. A8은 A7 source commit `51de6030a69a8e36fcf7e5790ef157eff6fedf00`, tree `3cd00b17bd8bd6aa8def213de1c5765c3611765d`, annotated tag object `417a318fb6a11a720e2c4102e92e39ea9ed676e9`, checksums와 Darwin archive·embedded manifest digest를 직접 검증한다. 기본 릴리스와 recovery는 Homebrew 토큰 생성 전에 같은 fail-closed verifier로 exact source-bound final·immutable Release, 중복 없는 11개 자산의 서버 digest와 크기·상태, 서버 `checksums.txt`, 8개 archive checksum과 API digest의 일치를 검증한다. Homebrew는 이 검증을 통과한 checksum만 사용해 live A7 Cask blob `a46b37d61bfd62a31fd5f4c6731d4f83fa1c868a`에서 canonical A8 Cask로 CAS 갱신하고 Formula blob `4ebc6c38925002dec00759823d4dd847a499818a`는 호출하거나 변경하지 않는다. 최종 A8 source commit·tree pin, 태그, 보호 환경 승인, 서명·공증, immutable GitHub Release와 Homebrew 반영은 아직 완료되지 않았다.
