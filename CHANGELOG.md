...
- **v0.50.93 companion A22 릴리스 준비와 OMP context 승격 증거 봉인** (2026-08-04): 릴리스 진입점을 annotated tag `v0.50.93`으로 제한하고 immutable A21 `v0.50.92`의 release ID `363679709`, source commit `b86fab067599f457261287552c5a9dd86460d7f4`, tree `64d32460859b325e55dcf9959a8478567c8849a7`, annotated tag object `fdc0e94fbe2325ed47cf7a3fd1792028c1f3bb75` 및 4개 플랫폼 archive·Darwin manifest digest를 직접 고정한다. 별도의 zero-parent annotated evidence tag에서 정확히 두 개의 report/attestation blob만 추출·해시 검증하고, secret 없는 job의 trusted-clock active 검증과 release job의 historical 재검증을 모두 통과한 동일 증거를 배포한다. canonical Darwin arm64 candidate와 GoReleaser raw binary는 `-trimpath`, commit time, full source commit/tree, release-compiled static policy가 같은 빌드 계약에서 exact SHA-256으로 일치해야 한다. Developer ID 서명 뒤의 배포 executable digest는 공개키 receipt로 검증한 release-lineage V1이 unsigned candidate digest에 연결하며, standalone/archive lineage pair의 byte equality와 raw Ed25519 서명을 포함한 정확히 15개 immutable release asset을 현재 릴리스 검증과 Homebrew 복구가 독립 재검증한다. 실제 protected-environment 승인, evidence tag pin 설정, 서명·공증, 게시와 Homebrew CAS 갱신은 태그 릴리스 단계에서 완료한다.
