...
- **v0.50.75 companion A6 릴리스 준비** (2026-07-19): 릴리스 워크플로와 복구 경로를 정확한 `v0.50.75` 태그에만 열고, A6가 변경 불가능한 `v0.50.74`의 source commit `b27252cb1148192a8ae1a95195c50e5f221453a4`, annotated tag object, checksums, Darwin archive와 manifest를 직접 검증하도록 계보를 확장했다. Homebrew는 현재 `v0.50.74` Cask blob `ceed648bfece4555e8310b6e894fedc847520960`에서 canonical `v0.50.75` Cask로만 CAS 갱신하며 `v0.50.71` Formula는 계속 동결한다. 이번 릴리스에는 CodeOps doctor/prepare, workspace sync 경고 분류, 중첩 에이전트 런타임 탐지 보강, Codex pane hook-result 수집과 attempt 격리 변경이 포함된다. 이 항목은 릴리스 준비 코드이며 태그 생성, 보호 환경 source pin 갱신, 실제 서명·공증·게시 완료를 뜻하지 않는다.
