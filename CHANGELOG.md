...
- **v0.50.95 companion A22 릴리스 준비와 OMP context 승격 증거 봉인** (2026-08-05): 아직 게시 전인 `v0.50.95`가 실패·미게시 상태로 보존한 `v0.50.93`·`v0.50.94`를 선행 릴리스로 사용하지 않고, published immutable A21 `v0.50.92`를 직접 계승하도록 A22 릴리스 좌표를 재수렴했다. A21 `v0.50.92`의 release ID `363679709`, source commit `b86fab067599f457261287552c5a9dd86460d7f4`, tree `64d32460859b325e55dcf9959a8478567c8849a7`, annotated tag object `fdc0e94fbe2325ed47cf7a3fd1792028c1f3bb75` 및 4개 플랫폼 archive·Darwin manifest digest는 그대로 고정한다. 승격은 zero-parent annotated `omp-context-evidence-v0.50.95` tag에 정확히 `omp-context-promotion-report.v1.json`과 `omp-context-promotion-attestation.v2.json`만 싣고 tag 게시 후 종료하며, GitHub Actions integration token으로 repository variable을 쓰지 않는다. 운영자가 evidence tag object·commit과 두 파일 SHA-256 좌표를 별도로 설정한 뒤에만 secret 없는 job의 trusted verifier가 고정 태그를 추출·검증하고, release job은 canonical static policy, source commit/tree, candidate digest와 fresh attestation을 재검증한다. 릴리스 lineage, canonical evidence 추출·검증, 실패 태그의 non-current·non-reusable 판정과 관련 오류 경로는 결정론적 동작 테스트로 보강하되 전체 커버리지 기준은 `83.0%`로 유지한다.
