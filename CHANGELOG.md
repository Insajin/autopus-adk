...
- **릴리스 아티팩트 publisher 서명 검증 도입 (SPEC-ADK-RELEASE-SIGNING-001)** (2026-07-17): checksum-only 신뢰 체인이 동일 origin(GitHub release assets)에서 아카이브와 `checksums.txt`를 함께 교체하는 공격에 취약했던 결함을 닫았다. producer는 `checksums.txt`에 ECDSA P-256(prime256v1, SHA-256, ASN.1/DER) detached 서명을 만들어 `checksums.txt.sig`로 게시하고(기존 cosign keyless bundle은 defense-in-depth로 존치), `auto update --self`와 `install.sh` 양쪽이 각각 바이너리·스크립트에 임베드된 pinned public key 집합으로 checksum을 신뢰하기 전에 서명을 fail-closed 검증한다(`pkg/selfupdate/signature.go::VerifyReleaseSignature`, `install.sh::verify_signature`). 검증은 와이어의 KeyID를 신뢰하지 않는 multi-trial 방식이다 — `ExpiresAt`가 지난 임베드 키는 시도 전에 제외되고 남은 키 전부로 검증을 시도해 하나라도 통과하면 진위로 인정, 전부 실패하면 거부한다(2-key 회전 과도기 지원). `install.sh`의 기존 체크섬 검증 도구 부재 시 건너뛰던 fail-open 결함도 fail-closed로 교체했다. **한계**: GitHub 릴리스는 immutable이라 `v0.50.71` 이하 기존 릴리스에는 `.sig` 자산을 소급 추가할 수 없다 — 이 변경이 포함된 다음 릴리스부터 서명이 시작되며, 그 전까지는 `install.sh`/`auto update --self`가 서명 자산이 없는 최신 릴리스에 대해 fail-closed로 설치/업데이트를 거부한다. `get.autopus.co` 원라이너 채널은 아직 미개통이며, 첫 서명 릴리스 이후 개통 순서다. 검증: `go test ./pkg/selfupdate/... -race` PASS(15개 신규 서명/pinned-key oracle 테스트 + Go↔openssl 양방향 상호운용 포함), `sh scripts/test-install-signing.sh`(합성 키로 정상/변조/공격자키/서명부재/만료/회전창/openssl부재 11개 시나리오) PASS.
