...
- **v0.50.78 companion A7 릴리스 준비(PENDING)** (2026-07-20): 릴리스 좌표를 `v0.50.78`로 옮기고, A7이 immutable `v0.50.77`을 직접 선행 릴리스로 검증하도록 source gate와 공개키 계보를 확장한다. A6의 source commit, annotated tag object, checksums, Darwin archive와 embedded manifest digest를 고정하며, Homebrew는 prior Cask blob `39b9b77eb51149ff87df7ad4f8fb3c5300b1302c`에서 canonical A7 Cask로만 CAS 갱신하고 Formula blob은 변경하지 않는다. 이 항목은 릴리스 준비 상태를 기록한다. `v0.50.78` 태그 생성, 보호 환경의 source commit·tree pin 갱신, 서명·공증, immutable GitHub Release 게시와 Homebrew 반영은 아직 완료되지 않았다.
