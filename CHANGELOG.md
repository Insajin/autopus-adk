...
- **v0.50.100 production execution smoke의 signed artifact 격리 staging** (2026-08-04): release runner에서 GoReleaser가 만든 최종 signed `auto`의 상위 workspace가 `nobody`에게 순회 불가능해, artifact 자체의 서명·모드가 정상이더라도 UID-isolated execution smoke가 올바르게 거부되던 경계를 수정했다. arm64 execution gate는 OMP canary root의 canonical `auto` 경로에 exact signed bytes를 root-owned `0555`로 materialize하고 source·staged inode의 stable digest를 실행 전후 검증한 뒤 `nobody` smoke를 수행한다. 원본 artifact의 manifest binding은 유지하며 staged copy는 성공·실패 모두에서 privilege-pinned cleanup으로 제거하므로, runner workspace 권한을 완화하거나 UID isolation을 우회하지 않는다.
