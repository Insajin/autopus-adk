#!/usr/bin/env bash
# Ephemeral local account boundary for the credential-bearing live canary.
...
create_release_canary_account() {
  ...
  /usr/bin/sudo -n /usr/bin/dscl . -create "/Users/${release_canary_user}" \
    RealName "$release_canary_marker" || fail 'mark dedicated live-canary account'
  /usr/bin/sudo -n /usr/bin/dscl . -create "/Users/${release_canary_user}" \
    UniqueID "$release_canary_uid" || fail 'set dedicated live-canary UID'
  /usr/bin/sudo -n /usr/bin/dscl . -create "/Users/${release_canary_user}" \
    PrimaryGroupID "$release_canary_gid" || fail 'set dedicated live-canary group'
  /usr/bin/sudo -n /usr/bin/dscl . -create "/Users/${release_canary_user}" \
    NFSHomeDirectory "$release_canary_home" || fail 'set dedicated live-canary home'
  /usr/bin/sudo -n /usr/bin/dscl . -create "/Users/${release_canary_user}" \
    UserShell /usr/bin/false || fail 'disable dedicated live-canary login shell'
  /usr/bin/sudo -n /usr/bin/dscl . -create "/Users/${release_canary_user}" \
    IsHidden 1 || fail 'hide dedicated live-canary account'
  verify_release_canary_account || fail 'dedicated live-canary account differs after creation'
  [[ "$(/usr/bin/sudo -n -u "$release_canary_user" /usr/bin/id -u)" == "$release_canary_uid" ]] ||
    fail 'dedicated live-canary account cannot execute'
}
...
remove_release_canary_account() {
  ...
  if /usr/bin/pgrep -u "$release_canary_uid" >/dev/null 2>&1; then
    ...
    for attempt in 1 2 3 4 5; do
      /bin/sleep 1
      /usr/bin/pgrep -u "$release_canary_uid" >/dev/null 2>&1 || break
    done
  fi
  if /usr/bin/pgrep -u "$release_canary_uid" >/dev/null 2>&1; then
  ...
  verify_release_canary_account || return 1
  /usr/bin/sudo -n /usr/bin/dscl . -delete "/Users/${release_canary_user}" || return 1
  /usr/bin/dscl . -read "/Users/${release_canary_user}" >/dev/null 2>&1 && return 1
  clear_release_canary_account_state
}
