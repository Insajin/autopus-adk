...
[[ "$release_tag" == 'v0.50.117' ]] || fail 'release tag is not exact A28'
...
seal_release_tag_ruleset() {
  ...
  ruleset_id=$(jq -er --arg name 'autopus-v0.50.117-release-authority' \
...
if [[ "$local_tag_preexisting" -eq 0 ]]; then
  env "${tag_git_config[@]}" git tag -s "$release_tag" "$source_commit" \
    -m "${release_tag} - A28 companion release"
