...
load_evidence() {
  ...
  for attempt in 1 2 3 4 5; do
    if credential_free_public_git "$public_home" -C "$public_root" fetch --quiet --force \
      --no-tags 'https://github.com/Insajin/autopus-adk.git' \
      "$evidence_ref:$evidence_ref"; then
      fetched=1
      break
    fi
    /bin/sleep 2
...
verify_evidence() {
  ...
  env -i PATH='/usr/bin:/bin:/usr/sbin:/sbin' HOME="$verifier_home" TMPDIR="$temp_dir" \
    ...
    --expected-signing-key-id "$expected_promotion_key_id"
}
...
publish_local_evidence() {
  ...
  case "$remote_tag_status" in
    0) fail 'evidence tag already exists' ;;
    2) ;;
    *) fail 'cannot inspect evidence tag before local publish' ;;
  esac
  ...
  [[ "$(git -C "$publish_root" rev-parse --verify FETCH_HEAD)" == "$evidence_source_commit" ]] ||
    fail 'fetched prep lock differs during local publish'
