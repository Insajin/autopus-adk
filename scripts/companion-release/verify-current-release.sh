...
if ! GH_TOKEN="$GITHUB_TOKEN" gh api \
  ...
  fail 'cannot read the exact A18 GitHub release'
fi
[[ -f "$release_json" && ! -L "$release_json" && -s "$release_json" ]] \
  || fail 'A18 GitHub release metadata is empty or unsafe'

expected_assets_json=$(printf '%s\n' "${EXPECTED_ASSETS[@]}" \
  | jq -Rsc 'split("\n") | map(select(length > 0))') \
  || fail 'cannot construct the expected A18 asset set'
if ! jq -e --arg tag "$RELEASE_TAG" --arg commit "$COMPANION_SOURCE_COMMIT" \
  ...
  fail 'A18 release is not exact, final, immutable, complete, and digest-bound'
fi
...
download_release_asset() {
  ...
  if ! GH_TOKEN="$GITHUB_TOKEN" gh api \
    ...
    fail "cannot download ${asset_name} from the exact A18 release"
  fi
...
if ! printf '%s' "$checksum_entries_json" | jq -e \
  ...
  ' >/dev/null; then
  fail 'checksums.txt does not describe exactly the eight A18 archives'
fi
...
env -i PATH="$PATH" HOME="${HOME:-/}" TMPDIR="${TMPDIR:-/tmp}" \
  ...
  || fail 'A18 release signature evidence is invalid'
...
printf 'current release evidence: exact immutable A18 release verified\n'
