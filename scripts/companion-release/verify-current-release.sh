...
GH_TOKEN="$GITHUB_TOKEN" gh api -H 'Accept: application/vnd.github+json' \
  ...
  fail 'cannot read exact A28 release'
[[ -s "$release_json" && ! -L "$release_json" ]] ||
  fail 'A28 release metadata is empty or unsafe'
...
' "$release_json" >/dev/null ||
  fail 'A28 release id, tag, source, author, state, assets, or digests differ'
...
download_release_asset() {
  ...
  GH_TOKEN="$GITHUB_TOKEN" gh api -H 'Accept: application/octet-stream' \
    ...
    fail "cannot download ${asset_name} from exact A28 release"
...
' >/dev/null || fail 'checksums.txt does not describe exactly eight A28 archives'
...
env -i PATH="$PATH" HOME="${HOME:-/}" TMPDIR="${TMPDIR:-/tmp}" \
  ...
  fail 'A28 checksum ECDSA/cosign evidence is invalid'
...
env -i PATH="$PATH" HOME="${HOME:-/}" TMPDIR="${TMPDIR:-/tmp}" \
  ...
  fail 'released A28 U-to-D lineage is invalid'
...
env -i PATH="$PATH" HOME="${HOME:-/}" TMPDIR="${TMPDIR:-/tmp}" \
  ...
  fail 'A28 historical-recovery OMP evidence is invalid'
...
printf 'current release evidence: exactly fifteen A28 normal release assets verified\n'
