...
{
  printf '%064d  autopus-adk_0.50.89_darwin_amd64.tar.gz\n' 1
  printf '%064d  autopus-adk_0.50.89_darwin_arm64.tar.gz\n' 2
  printf '%064d  autopus-adk_0.50.89_linux_amd64.tar.gz\n' 3
  printf '%064d  autopus-adk_0.50.89_linux_arm64.tar.gz\n' 4
} >"$checksums"

# A18 updates only the Cask from the exact A17 tap head and keeps Formula frozen.
source "$script_dir/publish-homebrew-formula-bridge-render.sh"
render_homebrew_cask "$temp/prior-cask.rb" 0.50.88 \
  '3bae06adc5c31281efd8d9fd016af0ab9158da54262e7f17fafd2ed9be6ff1b1' \
  'bf7a96a4ce34b58a940ab9813c2b797ec8f4a489c794b54d6b54097c5ecc4cce' \
  'e09b66b7a1683ef8764e59ca620242ee7eebbee573c0fec5232f9adb649f6909' \
  '31cc659ae347346204db2dae13368e6277b1e21f7e3a7c16dab160dd2b98176d'
[[ "$(git -C "$temp" hash-object "$temp/prior-cask.rb")" == \
   'd694773024a3a0a6719a925b36e809fc9c28f402' ]] \
  || fail 'rendered A17 Cask bytes differ from the pinned predecessor blob'
...
jq -n --arg content "$(base64 <"$temp/prior-cask.rb" | tr -d '\r\n')" \
  '{sha:"d694773024a3a0a6719a925b36e809fc9c28f402",content:$content}' >"$state/cask.json"
...
jq -n '{ref:"refs/heads/main",object:{type:"commit",sha:"a2c3e07311b216927225f27d8e7c6f46899cd85c",url:"https://example.invalid/prior-commit"}}' \
  >"$state/branch.json"
cp "$state/formula.json" "$temp/formula-before.json"
bridge_env=(PATH="$temp/bin:$PATH" MOCK_TAP_STATE="$state" GITHUB_REF_NAME=v0.50.89
  COMPANION_VERSION=0.50.89 COMPANION_HOMEBREW_POLICY=cask-only
...
[[ "$(<"$state/ref-update.calls")" == 1 &&
  ...
  || fail 'A18 did not update only the Cask'
...
[[ "$(<"$state/ref-update.calls")" == 1 &&
  ...
  || fail 'A18 Cask-only reconciler is not idempotent'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A18 accepted idempotent Cask bytes across concurrent Formula drift'
fi
...
[[ "$(<"$state/ref-update.calls")" == 1 &&
  ...
  || fail 'A18 mutated tap state after idempotent Formula drift'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A18 accepted a branch move after idempotent tree verification'
fi
rm -f -- "$state/idempotent-ref-race"
[[ "$(<"$state/ref-update.calls")" == 1 &&
   "$(jq -er '.object.sha' "$state/branch.json")" == \
     '4444444444444444444444444444444444444444' ]] \
  || fail 'A18 updated Cask during idempotent head verification'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A18 accepted a drifted Homebrew tap predecessor commit'
fi
[[ "$(<"$state/ref-update.calls")" == 1 ]] \
  || fail 'A18 updated Cask after predecessor commit drift'
jq -n '{ref:"refs/heads/main",object:{type:"commit",sha:"a2c3e07311b216927225f27d8e7c6f46899cd85c",url:"https://example.invalid/prior-commit"}}' \
  >"$state/branch.json"
jq -n --arg content "$(base64 <"$temp/frozen-formula.rb" | tr -d '\r\n')" \
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A18 accepted frozen Formula blob drift'
fi
[[ "$(<"$state/ref-update.calls")" == 1 ]] \
  || fail 'A18 mutated tap state after frozen Formula drift'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A18 accepted a concurrent Homebrew branch move'
fi
rm -f -- "$state/race-before-ref"
[[ "$(<"$state/ref-update.calls")" == 1 &&
   "$(jq -er '.object.sha' "$state/branch.json")" == \
     '4444444444444444444444444444444444444444' ]] \
  || fail 'A18 overwrote a concurrent Homebrew branch move'

# A concurrent Formula-changing commit must also win the race and reject A18.
jq -n --arg content "$(base64 <"$temp/frozen-formula.rb" | tr -d '\r\n')" \
  '{sha:"4ebc6c38925002dec00759823d4dd847a499818a",content:$content}' >"$state/formula.json"
jq -n '{ref:"refs/heads/main",object:{type:"commit",sha:"a2c3e07311b216927225f27d8e7c6f46899cd85c",url:"https://example.invalid/prior-commit"}}' \
  >"$state/branch.json"
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A18 accepted a concurrent Formula drift commit'
fi
rm -f -- "$state/formula-race-before-ref"
[[ "$(<"$state/ref-update.calls")" == 1 &&
   "$(jq -er '.sha' "$state/formula.json")" == \
     '6666666666666666666666666666666666666666' ]] \
  || fail 'A18 overwrote a concurrent Formula drift commit'
