...
{
  printf '%064d  autopus-adk_0.50.91_darwin_amd64.tar.gz\n' 1
  printf '%064d  autopus-adk_0.50.91_darwin_arm64.tar.gz\n' 2
  printf '%064d  autopus-adk_0.50.91_linux_amd64.tar.gz\n' 3
  printf '%064d  autopus-adk_0.50.91_linux_arm64.tar.gz\n' 4
} >"$checksums"

# A20 updates only the Cask from the exact A19 tap head and keeps Formula frozen.
source "$script_dir/publish-homebrew-formula-bridge-render.sh"
render_homebrew_cask "$temp/prior-cask.rb" 0.50.90 \
  '8fe1e6b04ee107ebb3af2de1e0b3a272f42ab352f15e62ef1a2ab9e0b067c630' \
  '60c4fe79c83a4b58482808a6ea9937389b9fa5c27015521df76705db98fd42b0' \
  '34582ef39c7b54fb9327e295e6cbb50107e3689120cf74f75b1f21a01eecc1dc' \
  '11a00378365069b97804d6af40435645f26271550d1ab58e7776346ec7d4a879'
[[ "$(git -C "$temp" hash-object "$temp/prior-cask.rb")" == \
   '0cd0afda423911ece3fbd099884851e2b4b371f2' ]] \
  || fail 'rendered A19 Cask bytes differ from the pinned predecessor blob'
...
jq -n --arg content "$(base64 <"$temp/prior-cask.rb" | tr -d '\r\n')" \
  '{sha:"0cd0afda423911ece3fbd099884851e2b4b371f2",content:$content}' >"$state/cask.json"
...
jq -n '{ref:"refs/heads/main",object:{type:"commit",sha:"b3d42d3dc86630e2e020cadbd58e1e15c1c70572",url:"https://example.invalid/prior-commit"}}' \
  >"$state/branch.json"
cp "$state/formula.json" "$temp/formula-before.json"
bridge_env=(PATH="$temp/bin:$PATH" MOCK_TAP_STATE="$state" GITHUB_REF_NAME=v0.50.91
  COMPANION_VERSION=0.50.91 COMPANION_HOMEBREW_POLICY=cask-only
...
[[ "$(<"$state/ref-update.calls")" == 1 &&
  ...
  || fail 'A20 did not update only the Cask'
...
[[ "$(<"$state/ref-update.calls")" == 1 &&
  ...
  || fail 'A20 Cask-only reconciler is not idempotent'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A20 accepted idempotent Cask bytes across concurrent Formula drift'
fi
...
[[ "$(<"$state/ref-update.calls")" == 1 &&
  ...
  || fail 'A20 mutated tap state after idempotent Formula drift'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A20 accepted a branch move after idempotent tree verification'
fi
rm -f -- "$state/idempotent-ref-race"
[[ "$(<"$state/ref-update.calls")" == 1 &&
   "$(jq -er '.object.sha' "$state/branch.json")" == \
     '4444444444444444444444444444444444444444' ]] \
  || fail 'A20 updated Cask during idempotent head verification'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A20 accepted a drifted Homebrew tap predecessor commit'
fi
[[ "$(<"$state/ref-update.calls")" == 1 ]] \
  || fail 'A20 updated Cask after predecessor commit drift'
jq -n '{ref:"refs/heads/main",object:{type:"commit",sha:"b3d42d3dc86630e2e020cadbd58e1e15c1c70572",url:"https://example.invalid/prior-commit"}}' \
  >"$state/branch.json"
jq -n --arg content "$(base64 <"$temp/frozen-formula.rb" | tr -d '\r\n')" \
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A20 accepted frozen Formula blob drift'
fi
[[ "$(<"$state/ref-update.calls")" == 1 ]] \
  || fail 'A20 mutated tap state after frozen Formula drift'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A20 accepted a concurrent Homebrew branch move'
fi
rm -f -- "$state/race-before-ref"
[[ "$(<"$state/ref-update.calls")" == 1 &&
   "$(jq -er '.object.sha' "$state/branch.json")" == \
     '4444444444444444444444444444444444444444' ]] \
  || fail 'A20 overwrote a concurrent Homebrew branch move'

# A concurrent Formula-changing commit must also win the race and reject A20.
jq -n --arg content "$(base64 <"$temp/frozen-formula.rb" | tr -d '\r\n')" \
  '{sha:"4ebc6c38925002dec00759823d4dd847a499818a",content:$content}' >"$state/formula.json"
jq -n '{ref:"refs/heads/main",object:{type:"commit",sha:"b3d42d3dc86630e2e020cadbd58e1e15c1c70572",url:"https://example.invalid/prior-commit"}}' \
  >"$state/branch.json"
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A20 accepted a concurrent Formula drift commit'
fi
rm -f -- "$state/formula-race-before-ref"
[[ "$(<"$state/ref-update.calls")" == 1 &&
   "$(jq -er '.sha' "$state/formula.json")" == \
     '6666666666666666666666666666666666666666' ]] \
  || fail 'A20 overwrote a concurrent Formula drift commit'
