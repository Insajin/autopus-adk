...
{
  printf '%064d  autopus-adk_0.50.87_darwin_amd64.tar.gz\n' 1
  printf '%064d  autopus-adk_0.50.87_darwin_arm64.tar.gz\n' 2
  printf '%064d  autopus-adk_0.50.87_linux_amd64.tar.gz\n' 3
  printf '%064d  autopus-adk_0.50.87_linux_arm64.tar.gz\n' 4
} >"$checksums"

# A16 updates only the Cask from the exact A15 tap head and keeps Formula frozen.
source "$script_dir/publish-homebrew-formula-bridge-render.sh"
render_homebrew_cask "$temp/prior-cask.rb" 0.50.86 \
  '41e2a371c89567ff862d5f47179c838cb3aefd83abeb0ff769e58b12579676e3' \
  '84ea326a10c860af82663db1c87a8a15bdee492143d77a02ad86a0b3ba930f8f' \
  'cae69dd8828cb2c12ba0d312c3f4dbc034104c1b4b9cee6cddf18eebe6430cb6' \
  '9e943908dabf910e9f3072f838a99dec3c9d4952d9058bfbc1b71cd78e3f29eb'
[[ "$(git -C "$temp" hash-object "$temp/prior-cask.rb")" == \
   'f9baefd8723dad6afb3d60999bde44d3913ecb10' ]] \
  || fail 'rendered A15 Cask bytes differ from the pinned predecessor blob'
...
jq -n --arg content "$(base64 <"$temp/prior-cask.rb" | tr -d '\r\n')" \
  '{sha:"f9baefd8723dad6afb3d60999bde44d3913ecb10",content:$content}' >"$state/cask.json"
...
jq -n '{ref:"refs/heads/main",object:{type:"commit",sha:"bb84d874af4c9187603f36c3ca06460c90b7caea",url:"https://example.invalid/prior-commit"}}' \
  >"$state/branch.json"
cp "$state/formula.json" "$temp/formula-before.json"
bridge_env=(PATH="$temp/bin:$PATH" MOCK_TAP_STATE="$state" GITHUB_REF_NAME=v0.50.87
  COMPANION_VERSION=0.50.87 COMPANION_HOMEBREW_POLICY=cask-only
...
[[ "$(<"$state/ref-update.calls")" == 1 &&
  ...
  || fail 'A16 did not update only the Cask'
...
[[ "$(<"$state/ref-update.calls")" == 1 &&
  ...
  || fail 'A16 Cask-only reconciler is not idempotent'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A16 accepted idempotent Cask bytes across concurrent Formula drift'
fi
...
[[ "$(<"$state/ref-update.calls")" == 1 &&
  ...
  || fail 'A16 mutated tap state after idempotent Formula drift'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A16 accepted a branch move after idempotent tree verification'
fi
rm -f -- "$state/idempotent-ref-race"
[[ "$(<"$state/ref-update.calls")" == 1 &&
   "$(jq -er '.object.sha' "$state/branch.json")" == \
     '4444444444444444444444444444444444444444' ]] \
  || fail 'A16 updated Cask during idempotent head verification'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A16 accepted a drifted Homebrew tap predecessor commit'
fi
[[ "$(<"$state/ref-update.calls")" == 1 ]] \
  || fail 'A16 updated Cask after predecessor commit drift'
jq -n '{ref:"refs/heads/main",object:{type:"commit",sha:"bb84d874af4c9187603f36c3ca06460c90b7caea",url:"https://example.invalid/prior-commit"}}' \
  >"$state/branch.json"
jq -n --arg content "$(base64 <"$temp/frozen-formula.rb" | tr -d '\r\n')" \
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A16 accepted frozen Formula blob drift'
fi
[[ "$(<"$state/ref-update.calls")" == 1 ]] \
  || fail 'A16 mutated tap state after frozen Formula drift'
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A16 accepted a concurrent Homebrew branch move'
fi
rm -f -- "$state/race-before-ref"
[[ "$(<"$state/ref-update.calls")" == 1 &&
   "$(jq -er '.object.sha' "$state/branch.json")" == \
     '4444444444444444444444444444444444444444' ]] \
  || fail 'A16 overwrote a concurrent Homebrew branch move'

# A concurrent Formula-changing commit must also win the race and reject A16.
jq -n --arg content "$(base64 <"$temp/frozen-formula.rb" | tr -d '\r\n')" \
  '{sha:"4ebc6c38925002dec00759823d4dd847a499818a",content:$content}' >"$state/formula.json"
jq -n '{ref:"refs/heads/main",object:{type:"commit",sha:"bb84d874af4c9187603f36c3ca06460c90b7caea",url:"https://example.invalid/prior-commit"}}' \
  >"$state/branch.json"
...
if env "${bridge_env[@]}" bash "$script_dir/publish-homebrew-formula-bridge.sh" \
  >/dev/null 2>&1; then
  fail 'A16 accepted a concurrent Formula drift commit'
fi
rm -f -- "$state/formula-race-before-ref"
[[ "$(<"$state/ref-update.calls")" == 1 &&
   "$(jq -er '.sha' "$state/formula.json")" == \
     '6666666666666666666666666666666666666666' ]] \
  || fail 'A16 overwrote a concurrent Formula drift commit'
