...
if env -i PATH="$PATH" HOME="${HOME-}" TMPDIR="${TMPDIR:-/tmp}" \
  GITHUB_REF_NAME=v0.50.104 GITHUB_SHA=bad COMPANION_SOURCE_TREE=bad OMP_CONTEXT_STATIC_POLICY_B64=eA \
...
if env -i PATH="$PATH" HOME="${HOME-}" TMPDIR="${TMPDIR:-/tmp}" \
  COMPANION_RELEASE_TAG=v0.50.104 GITHUB_SHA=bad COMPANION_SOURCE_TREE=bad OMP_CONTEXT_STATIC_POLICY_B64=eA \
  /bin/bash "$candidate_builder" "$temp_dir/builder-output" >/dev/null 2>"$builder_error"; then
