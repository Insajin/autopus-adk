...
for required in \
  'omp-context-evidence-v0.50.116' \
...
printf '{"fixture":"fresh-v0.50.116-report"}' >"$report"
printf '{"fixture":"fresh-v0.50.116-attestation"}' >"$attestation"
...
GIT_COMMITTER_DATE='2026-08-31T00:00:01Z' \
  git -C "$repo" tag -am 'fresh A24 evidence' omp-context-evidence-v0.50.116 "$commit"
tag_object=$(git -C "$repo" rev-parse refs/tags/omp-context-evidence-v0.50.116)
...
git -C "$repo" update-ref -d refs/tags/omp-context-evidence-v0.50.116
if (cd "$repo" && run_helper "$temp/stale-ref"); then
  fail 'stale v0.50.108 evidence ref passed as A23'
fi
git -C "$repo" update-ref refs/tags/omp-context-evidence-v0.50.116 "$tag_object"

git -C "$repo" update-ref refs/tags/omp-context-evidence-v0.50.116 "$commit"
...
git -C "$repo" tag -fam 'drifted evidence' omp-context-evidence-v0.50.116 "$extra_commit"
extra_tag=$(git -C "$repo" rev-parse refs/tags/omp-context-evidence-v0.50.116)
...
git -C "$repo" tag -fam 'parented evidence' omp-context-evidence-v0.50.116 "$child"
parented_tag=$(git -C "$repo" rev-parse refs/tags/omp-context-evidence-v0.50.116)
...
env COMPANION_RELEASE_TAG='v0.50.116' COMPANION_ARTIFACT="$artifact" \
...
if env COMPANION_RELEASE_TAG='v0.50.116' COMPANION_ARTIFACT="$artifact" \
  COMPANION_PLATFORM='darwin' COMPANION_ARCHITECTURE='arm64' OMP_CONTEXT_STATIC_POLICY_B64='e30' \
