...
GIT_COMMITTER_DATE='2026-08-04T00:00:01Z' \
  git -C "$repo" tag -am 'fixture evidence' omp-context-evidence-v0.50.102 "$commit"
tag_object=$(git -C "$repo" rev-parse refs/tags/omp-context-evidence-v0.50.102)
...
git -C "$repo" update-ref refs/tags/omp-context-evidence-v0.50.102 "$commit"
...
git -C "$repo" tag -fam 'parented evidence' omp-context-evidence-v0.50.102 "$child"
parented_tag=$(git -C "$repo" rev-parse refs/tags/omp-context-evidence-v0.50.102)
...
env COMPANION_RELEASE_TAG='v0.50.102' COMPANION_ARTIFACT="$artifact" \
  COMPANION_PLATFORM='darwin' COMPANION_ARCHITECTURE='arm64' \
  OMP_CONTEXT_CANDIDATE_ARTIFACT_SHA256="$artifact_sha" \
  bash "$binary_helper"
if env COMPANION_RELEASE_TAG='v0.50.102' COMPANION_ARTIFACT="$artifact" \
  COMPANION_PLATFORM='darwin' COMPANION_ARCHITECTURE='arm64' \
