...
setup_fixture() {
  ...
  env "${git_env[@]}" git -C "$work" tag -s omp-context-evidence-v0.50.97 \
    "$evidence_commit" -m evidence
  evidence_tag_object=$(git -C "$work" rev-parse refs/tags/omp-context-evidence-v0.50.97)
  git -C "$work" push --quiet origin refs/tags/omp-context-evidence-v0.50.97
  git -C "$work" push --quiet origin \
    "$prep_lock_commit:refs/heads/omp-context-evidence-v0.50.97-source"
...
while read -r _ _ ref; do
  if [[ "$ref" == 'refs/tags/v0.50.97' ]]; then
...
run_publisher() {
  ...
  env "${git_env[@]}" PATH="$fixture/bin:$PATH" MOCK_RELEASE_PREP_STATE="$state" \
    ...
    bash "$publisher" Insajin/autopus-adk adk-companion-release v0.50.97 \
...
[[ "$(cd "$work" && "$lock_verifier" \
  refs/heads/omp-context-evidence-v0.50.97-source "$prep_lock_commit" "$fixture/report")" == \
...
jq -e '.mode == "committed" and .release_tag == "v0.50.97" and
...
[[ "$(git -C "$work" ls-remote --refs origin refs/tags/v0.50.97 | cut -f2)" == 'refs/tags/v0.50.97' ]] \
...
jq -e 'any(.[]; .type == "tag" and .name == "v0.50.97")' \
...
jq -e '.mode == "reconciled" and .release_tag == "v0.50.97"' \
...
[[ -z "$(git -C "$work" ls-remote --refs origin refs/tags/v0.50.97)" ]] ||
...
[[ "$(git -C "$work" ls-remote --refs origin \
  refs/heads/omp-context-evidence-v0.50.97-source | cut -f1)" == "$prep_lock_commit" ]] ||
...
jq -n --arg source "$source_commit" \
  '[{id:995,tag_name:"v0.50.95-collision",target_commitish:$source,name:"v0.50.97",body:"collision",draft:true,prerelease:false,author:{id:204883817},assets:[]}]' \
  >"$state/releases.json"
...
jq -n --arg source "$source_commit" \
  '[{id:997,tag_name:"v0.50.97",target_commitish:$source,name:"v0.50.97",body:"unowned",draft:true,prerelease:false,author:{id:42},assets:[]}]' \
  >"$state/releases.json"
if (cd "$work" && run_publisher >/dev/null 2>&1); then
...
git -C "$work" push --quiet origin :refs/heads/omp-context-evidence-v0.50.97-source
cp "$state/repository-variables.json" "$fixture/repository-before.json"
cp "$state/environment-variables.json" "$fixture/environment-before.json"
if (cd "$work" && run_publisher >/dev/null 2>&1); then
