...
contains "$publisher_lib" 'gh variable list "$@" --limit 1000'
...
setup_fixture() {
  ...
  env "${git_env[@]}" git -C "$work" tag -s omp-context-evidence-v0.50.96 \
    "$evidence_commit" -m evidence
  evidence_tag_object=$(git -C "$work" rev-parse refs/tags/omp-context-evidence-v0.50.96)
  git -C "$work" push --quiet origin refs/tags/omp-context-evidence-v0.50.96
  git -C "$work" push --quiet origin \
    "$prep_lock_commit:refs/heads/omp-context-evidence-v0.50.96-source"
...
while read -r _ _ ref; do
  if [[ "$ref" == 'refs/tags/v0.50.96' ]]; then
...
run_publisher() {
  ...
  env "${git_env[@]}" PATH="$fixture/bin:$PATH" MOCK_RELEASE_PREP_STATE="$state" \
    ...
    bash "$publisher" Insajin/autopus-adk adk-companion-release v0.50.96 \
...
[[ "$(cd "$work" && "$lock_verifier" \
  refs/heads/omp-context-evidence-v0.50.96-source "$prep_lock_commit" "$fixture/report")" == \
...
jq -e '.mode == "committed" and .release_tag == "v0.50.96" and
  .source_commit == $source and .github_release_id == 996' \
  --arg source "$source_commit" "$fixture/result.json" >/dev/null || fail 'success receipt differs'
[[ "$(git -C "$work" ls-remote --refs origin refs/tags/v0.50.96 | cut -f2)" == 'refs/tags/v0.50.96' ]] \
  || fail 'success did not publish release tag'
[[ "$(jq 'length' "$state/repository-variables.json")" == '9' &&
   "$(jq 'length' "$state/environment-variables.json")" == '9' ]] \
  || fail 'success did not converge both variable scopes'
jq -e 'any(.[]; .type == "tag" and .name == "v0.50.96")' \
  "$state/deployment-policies.json" >/dev/null ||
  fail 'success did not install exact tag deployment policy'
jq -e 'length == 1 and .[0].id == 996 and .[0].draft == true and
  .[0].author.id == 204883817 and
  (.[0].body | fromjson | .schema_version == "autopus.adk_release_reservation.v1")' \
  "$state/releases.json" >/dev/null || fail 'success did not retain the owned draft reservation'
[[ "$(sed -n '$p' "$state/calls.log")" == 'release-tag-push' ]] ||
  fail 'release tag was not the final external mutation'
[[ -z "$(git -C "$work" ls-remote --refs origin \
  refs/heads/omp-context-evidence-v0.50.96-source)" ]] ||
  fail 'success did not atomically retire the prep lock'
...
jq -e '.mode == "reconciled" and .release_tag == "v0.50.96"' \
  "$fixture/reconciled.json" >/dev/null || fail 'committed release did not reconcile'
[[ "$(wc -l <"$state/calls.log" | tr -d ' ')" == "$calls_before_reconcile" ]] ||
  fail 'reconciliation performed a remote mutation'
...
[[ -z "$(git -C "$work" ls-remote --refs origin refs/tags/v0.50.96)" ]] ||
...
[[ "$(git -C "$work" ls-remote --refs origin \
  refs/heads/omp-context-evidence-v0.50.96-source | cut -f1)" == "$prep_lock_commit" ]] ||
...
jq -n --arg source "$source_commit" \
  '[{id:995,tag_name:"v0.50.95-collision",target_commitish:$source,name:"v0.50.96",body:"collision",draft:true,prerelease:false,author:{id:204883817},assets:[]}]' \
  >"$state/releases.json"
...
jq -n --arg source "$source_commit" \
  '[{id:997,tag_name:"v0.50.96",target_commitish:$source,name:"v0.50.96",body:"unowned",draft:true,prerelease:false,author:{id:42},assets:[]}]' \
  >"$state/releases.json"
if (cd "$work" && run_publisher >/dev/null 2>&1); then
...
git -C "$work" push --quiet origin :refs/heads/omp-context-evidence-v0.50.96-source
cp "$state/repository-variables.json" "$fixture/repository-before.json"
cp "$state/environment-variables.json" "$fixture/environment-before.json"
if (cd "$work" && run_publisher >/dev/null 2>&1); then
