...
setup_fixture() {
  ...
  env "${git_env[@]}" git -C "$work" tag -s omp-context-evidence-v0.50.107 \
    "$evidence_commit" -m evidence
  evidence_tag_object=$(git -C "$work" rev-parse refs/tags/omp-context-evidence-v0.50.107)
  git -C "$work" push --quiet origin refs/tags/omp-context-evidence-v0.50.107
  git -C "$work" push --quiet origin "$prep_lock_commit:refs/heads/omp-context-evidence-v0.50.107-source"; git -C "$work" tag -d omp-context-evidence-v0.50.107 >/dev/null; git -C "$work" reflog expire --expire=now --all; git -C "$work" gc --prune=now; if git -C "$work" cat-file -e "$evidence_tag_object" 2>/dev/null; then fail 'fixture retained local evidence tag object'; fi
...
while read -r _ _ ref; do
  if [[ "$ref" == 'refs/tags/v0.50.107' ]]; then
...
run_publisher() {
  ...
  env "${git_env[@]}" PATH="$fixture/bin:$PATH" MOCK_RELEASE_PREP_STATE="$state" \
    ...
    bash "$publisher" Insajin/autopus-adk adk-companion-release v0.50.107 \
...
[[ "$(cd "$work" && "$lock_verifier" \
  refs/heads/omp-context-evidence-v0.50.107-source "$prep_lock_commit" "$fixture/report")" == \
...
jq -e '.mode == "committed" and .release_tag == "v0.50.107" and .source_commit == $source and .github_release_id == 996' --arg source "$source_commit" "$fixture/result.json" >/dev/null || fail 'success receipt differs'
[[ "$(git -C "$work" ls-remote --refs origin refs/tags/v0.50.107 | cut -f2)" == 'refs/tags/v0.50.107' ]] || fail 'success did not publish release tag'
...
jq -e 'any(.[]; .type == "tag" and .name == "v0.50.107")' "$state/deployment-policies.json" >/dev/null || fail 'success did not install exact tag deployment policy'
...
[[ -z "$(git -C "$work" ls-remote --refs origin \
  refs/heads/omp-context-evidence-v0.50.107-source)" ]] || fail 'success did not atomically retire the prep lock'
...
jq -e '.mode == "reconciled" and .release_tag == "v0.50.107"' "$fixture/reconciled.json" >/dev/null || fail 'committed release did not reconcile'
...
[[ -z "$(git -C "$work" ls-remote --refs origin refs/tags/v0.50.107)" ]] ||
  fail 'failed publish created release tag'
[[ -z "$(git -C "$work" ls-remote --refs origin \
  refs/heads/omp-context-evidence-v0.50.107-source)" ]] ||
...
[[ "$(git -C "$work" ls-remote --refs origin \
  refs/heads/omp-context-evidence-v0.50.107-source | cut -f1)" == "$prep_lock_commit" ]] ||
...
jq -e 'any(.[]; .type == "tag" and .name == "v0.50.107")' \
...
[[ -z "$(git -C "$work" ls-remote --refs origin \
  refs/heads/omp-context-evidence-v0.50.107-source)" ]] || fail 'successful eventual-consistency retry retained the prep lock'
...
jq -n --arg source "$source_commit" \
  '[{id:995,tag_name:"v0.50.95-collision",target_commitish:$source,name:"v0.50.107",body:"collision",draft:true,prerelease:false,author:{id:204883817},assets:[]}]' \
  >"$state/releases.json"
...
jq -n --arg source "$source_commit" \
  '[{id:997,tag_name:"v0.50.107",target_commitish:$source,name:"v0.50.107",body:"unowned",draft:true,prerelease:false,author:{id:42},assets:[]}]' \
  >"$state/releases.json"
if (cd "$work" && run_publisher >/dev/null 2>&1); then
...
git -C "$work" push --quiet origin :refs/heads/omp-context-evidence-v0.50.107-source
cp "$state/repository-variables.json" "$fixture/repository-before.json"
cp "$state/environment-variables.json" "$fixture/environment-before.json"
if (cd "$work" && run_publisher >/dev/null 2>&1); then
