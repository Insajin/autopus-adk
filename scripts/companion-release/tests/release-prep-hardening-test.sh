...
contains "$prep" "readonly release_tag='v0.50.115'"; contains "$prep" "expected_go_toolchain='go1.26.6'"
...
printf '%s\n' '[{"id":596,"type":"tag","name":"v0.50.115"}]' >"$ruleset_state/deployment-policies.json"
...
setup_fixture() {
  ...
  printf '%s\n' '[{"id":596,"type":"tag","name":"v0.50.115"}]' >"$state/deployment-policies.json"
  printf '%s\n' '[]' >"$state/releases.json"
...
EOF
  ...
  env "${git_env[@]}" git -C "$work" tag -s omp-context-evidence-v0.50.115 "$evidence_commit" -m evidence
  evidence_tag_object=$(git -C "$work" rev-parse refs/tags/omp-context-evidence-v0.50.115)
  git -C "$work" push --quiet origin refs/tags/omp-context-evidence-v0.50.115
  git -C "$work" push --quiet origin "$prep_lock_commit:refs/heads/omp-context-evidence-v0.50.115-source"
  git -C "$work" tag -d omp-context-evidence-v0.50.115 >/dev/null
...
while read -r _ _ ref; do
  if [[ "$ref" == 'refs/tags/v0.50.115' ]]; then
...
run_publisher() {
  ...
  env "${git_env[@]}" PATH="$fixture/bin:$PATH" MOCK_RELEASE_PREP_STATE="$state" \
    ...
    bash "$publisher" Insajin/autopus-adk adk-companion-release v0.50.115 "$source_commit" "$source_tree" \
...
jq -e '.mode == "committed" and .release_tag == "v0.50.115" and .promotion_signing_key_id == "omp-context-promotion-2026-q3-k3"' "$fixture/result.json" >/dev/null || fail 'success receipt differs'
committed_tag=$(git -C "$work" rev-parse refs/tags/v0.50.115)
...
[[ -z "$(git -C "$work" ls-remote --refs origin refs/tags/v0.50.115)" ]] || fail 'rollback created a release tag'
