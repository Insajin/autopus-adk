...
if [[ "$(uname -s)" == 'Darwin' ]]; then
  ...
  go build -trimpath \
    -ldflags '-X github.com/insajin/autopus-adk/pkg/version.version=0.50.84' \
    -o "$artifact" "$repo/cmd/auto"
  "$gate" --artifact "$artifact" --expected-version 0.50.84 \
    --architecture "$architecture" --timeout 15s
  if "$gate" --artifact "$artifact" --expected-version 0.50.83 \
