  ...
  api)
    ...
    case "$method:$endpoint" in
      ...
      'GET:repos/Insajin/autopus-adk/releases?per_page=100')
        ...
        ;;
      GET:repos/Insajin/autopus-adk/releases/tags/v0.50.105)
        jq -ce '.[] | select(.tag_name == "v0.50.105")' "$state/releases.json"
        ;;
      POST:repos/Insajin/autopus-adk/releases)
        [[ "$field_tag" == 'v0.50.105' && "$field_target" =~ ^[0-9a-f]{40}$ &&
           "$field_release_name" == 'v0.50.105' &&
      ...
      POST:repos/Insajin/autopus-adk/environments/adk-companion-release/deployment-branch-policies)
        [[ "$field_name" == 'v0.50.105' && "$field_type" == 'tag' ]] || exit 65
        write_count
        jq '[.[] | select(.name != "v0.50.105")] + [{id:596,type:"tag",name:"v0.50.105"}]' \
        ...
        printf '%s\n' '{"id":596,"type":"tag","name":"v0.50.105"}'
        ;;
