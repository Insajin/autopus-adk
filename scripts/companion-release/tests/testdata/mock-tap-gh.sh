...
readonly prior_commit='f62ca4883c8770104291185bc30184aa5fb1af50'
...
case "$endpoint" in
  ...
  *git/commits*)
    ...
    jq -e --arg tree "$target_tree" --arg parent "$prior_commit" '
      .message == "Publish signed Cask for v0.50.88" and .tree == $tree and
      .parents == [$parent]
    ' "$input" >/dev/null || exit 65
    increment commit-create
    jq -n --arg sha "$target_commit" --arg tree "$target_tree" \
      --arg parent "$prior_commit" \
      '{sha:$sha,message:"Publish signed Cask for v0.50.88",tree:{sha:$tree},
