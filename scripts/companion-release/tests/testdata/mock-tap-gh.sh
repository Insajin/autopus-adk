...
readonly prior_commit='624e317d0433fe0efc91efb5dc5e8708ed50a22d'
...
case "$endpoint" in
  ...
  *git/commits*)
    ...
    jq -e --arg tree "$target_tree" --arg parent "$prior_commit" '
      .message == "Publish signed Cask for v0.50.83" and .tree == $tree and
      .parents == [$parent]
    ' "$input" >/dev/null || exit 65
    increment commit-create
    jq -n --arg sha "$target_commit" --arg tree "$target_tree" \
      --arg parent "$prior_commit" \
      '{sha:$sha,message:"Publish signed Cask for v0.50.83",tree:{sha:$tree},
