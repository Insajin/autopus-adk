...
readonly prior_commit='d8dc4c78f42a7c5e30176334b607b036be3bd677'
...
case "$endpoint" in
  ...
  *git/commits*)
    ...
    jq -e --arg tree "$target_tree" --arg parent "$prior_commit" '
      .message == "Publish signed Cask for v0.50.85" and .tree == $tree and
      .parents == [$parent]
    ' "$input" >/dev/null || exit 65
    increment commit-create
    jq -n --arg sha "$target_commit" --arg tree "$target_tree" \
      --arg parent "$prior_commit" \
      '{sha:$sha,message:"Publish signed Cask for v0.50.85",tree:{sha:$tree},
