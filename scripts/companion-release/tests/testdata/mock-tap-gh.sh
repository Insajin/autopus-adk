...
readonly prior_commit='b3d42d3dc86630e2e020cadbd58e1e15c1c70572'
...
case "$endpoint" in
  ...
  *git/commits*)
    ...
    jq -e --arg tree "$target_tree" --arg parent "$prior_commit" '
      .message == "Publish signed Cask for v0.50.91" and .tree == $tree and
      .parents == [$parent]
    ' "$input" >/dev/null || exit 65
    increment commit-create
    jq -n --arg sha "$target_commit" --arg tree "$target_tree" \
      --arg parent "$prior_commit" \
      '{sha:$sha,message:"Publish signed Cask for v0.50.91",tree:{sha:$tree},
