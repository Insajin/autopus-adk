...
- **User-visible outcome**: 워크스페이스 어디서든 `auto sync verify`를 실행하면 (1) 루트+직속 nested repo의 dirty·tracked-but-ignored 경로를 열거하고, (2) 모든 경로를 Phase A/B 후보·blocked generated/runtime·미분류로 완전 분할하며, (3) 위반을 WARN으로 출력하고, (4) 4개 셸에서 안전한 상대 경로만 포함하는 결정적 `git -C <repo> add -- <files>` 계획을 제안한다. `--spec`은 워크스페이스 전체에서 정확히 한 host를 검증하고 소유 파일만 계획한다. read-only(변이 0), 기본 exit 0, `--strict`면 위반 존재 시 exit 1.
...
- **Completion evidence**: S1~S13 fixture oracle, focused race, vet, 전체 CLI 테스트, 실제 meta workspace read-only `--spec` 실행, diff/file-size/tracked-ignored gate가 모두 2026-07-17에 통과했다.
...
WHEN 커밋 경계 repo가 열거될 때, THE SYSTEM SHALL 각 repo에서 `git --no-optional-locks status --porcelain=v1 -z --untracked-files=all`과 NUL-delimited `ls-files -c -i --exclude-standard -z`로 dirty·tracked-but-ignored 경로를 수집하고 rename의 source/destination을 보존하며, repository 상태를 변경하는 어떤 git 명령도 실행하지 않아야 한다.
- EARS type: Event
- Priority: Must
- 관측 지점: repo별 dirty 파일 목록 + 실행 후 git 상태 불변

### REQ-003: Phase 분류
WHEN dirty 파일이 repo에 귀속될 때, THE SYSTEM SHALL canonical workspace 정책의 root keep 집합(`AGENTS.md`, `ARCHITECTURE.md`, `CLAUDE.md`, `autopus.yaml`, `opencode.json`, `.mcp.json`, `.autopus/context/constraints.yaml`, `.autopus/project/**`, `.autopus/specs/**`, `.autopus/learnings/pipeline.jsonl`, human-maintained root Markdown)은 Phase B로, nested source는 Phase A로, generated/runtime·tracked-but-ignored는 blocked로, 나머지는 미분류로 배타적 완전 분할해야 한다.
- EARS type: Event
- Priority: Must
- 관측 지점: 파일별 Phase 라벨
...
WHEN 한 repo에 스테이징된 파일과 미스테이징 파일이 공존하거나, `--spec`이 설정되고 workspace 전체 inventory가 대상 SPEC 소유가 아닌 경로를 포함할 때, THE SYSTEM SHALL blocked·미분류를 포함한 무관 경로를 workspace-relative로 나열해야 한다.
- EARS type: Event
- Priority: Must
- 관측 지점: 혼입 의심 파일을 나열한 WARN 라인

### REQ-007: --spec 소유/무관 분리
WHERE `--spec SPEC-ID`가 제공될 때, THE SYSTEM SHALL SPEC-ID를 `^SPEC-[A-Z0-9-]+$`로 검증하고 대상 SPEC의 `plan.md`·`spec.md` 참조를 workspace-relative path로 정규화해 모든 repo의 dirty 파일을 소유/무관으로 분리하며, Phase A/B 계획에는 소유 후보만 남겨야 한다.
- EARS type: State
- Priority: Must
- 관측 지점: 두 그룹으로 분리된 파일 목록

### REQ-008: 결정적 커밋 계획 출력
WHEN 분류가 완료될 때, THE SYSTEM SHALL Phase A 모듈 repo를 상대 경로 알파벳순으로 정렬한 뒤 Phase B 메타를 배치하고, 공통 셸에서 literal token으로 안전한 path만 명시적 `git -C <repo> add -- <files>` 라인에 포함하며 Lore 커밋 리마인더 1줄을 출력해야 한다.
- EARS type: Event
- Priority: Must
- 관측 지점: 순서가 고정된 계획 블록

### REQ-009: read-only + exit 계약
WHERE `--strict`가 설정되고 경계·소유권·blocked·미분류 경고가 하나라도 출력됐을 때 THE SYSTEM SHALL exit code 1로 종료하고, 그 외 모든 경우 THE SYSTEM SHALL 어떤 변이도 없이 exit 0으로 종료해야 한다.
- EARS type: State
- Priority: Must
...
### REQ-012: SPEC host·문서 fail-closed
WHERE `--spec`이 설정될 때, THE SYSTEM SHALL 전체 workspace repo에서 regular/non-symlink SPEC host가 정확히 하나임을 `Lstat`과 repo containment로 검증하고, duplicate·symlink component·missing/non-regular/unreadable `spec.md` 또는 `plan.md`를 계획 렌더 전에 거부해야 한다.
- EARS type: State
- Priority: Must
- 관측 지점: duplicate/symlink/missing document 오류 + 출력 0

### REQ-013: 무부작용·진단 비노출
THE SYSTEM SHALL 실행 전후 각 repo의 HEAD, porcelain 상태, index bytes/hash/mtime를 보존하고, Git 실패 시 stderr·절대 경로·민감 문자열을 오류에 포함하지 않아야 한다.
- EARS type: Ubiquitous
- Priority: Must
- 관측 지점: index 불변 oracle + sanitized Git diagnostic
...
- `[NEW] internal/cli/sync_verify_discover.go` — 메타 루트 상향 탐색 + `setup.DetectMultiRepo` + optional-lock-free NUL Git inventory와 rename 파싱(REQ-001·002·013).
- `[NEW] internal/cli/sync_verify_policy.go`·`sync_verify_classify.go` — canonical root keep/generated-runtime 정책, 완전 분할, misplacement·SPEC-module·혼입 경고(REQ-003~006).
- `[NEW] internal/cli/sync_verify_spec_host.go`·`sync_verify_spec_ownership.go` — exactly-one host containment, regular 문서 읽기, workspace 전체 소유/무관 분리(REQ-007·011·012).
...
| REQ-012 (Must) | T11 | S12 | INV-008 |
| REQ-013 (Must) | T12 | S8, S13 | INV-007, INV-009 |
