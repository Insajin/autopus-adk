...
|---------------|-------|------|----------|-------|--------|-----------|--------|---------|----------|
| A1 | implementation_assumption | high | gateway ↔ OMP 18.1.13 `[remote, snapcompact]` manual compaction | Two T0 isolated-UID probes; latest source 81a5361e | full 40-call/18-attempt records; exact checkpoint lifecycle and complete maintenance coverage; no publication | isolated UID, body-free retained file | FAIL | Both runs completed five primary calls and stopped at sequence 6. Latest attempt identifies pre_ack_out_of_order (61ms), confirming repeated pre-checkpoint conflicts with single-pre assumption. First-method failure cause and remote completion remain unknown. No third live retry or oracle implementation before checkpoint contract revision/review. | retained probe-c046b270bbac69ffb1ec736409a138da.jsonl; research T0 attempt 2 |
...
- [ ] **T1: effective chain·방법·거부·유지비 관측·product 동등성.** `workflow_context_runtime_product_overlay.go`, managed RPC preflight/readback, live fixture 및 `workflow_context_runtime_managed_rpc_{run,boundary,completion,product}.go`에서 REQ-METHOD/PRODUCT 계약을 적용한다. 현재 native-end 검증은 `pipelineOMPActiveProbe.validNativeEnd`와 `pipelineOMPActiveNativeEndShape`(`pipeline_omp_context_active_lifecycle_frames.go`)로 나뉘므로 이 경로와 manualCompact의 start 검사를 함께 다룬다. `[NEW] pipeline_omp_context_active_compaction_method.go`에서 verified chain·method/refusal·maintenance 관측성을 판정하며, `pipeline_omp_context_active_transcript.go`의 기존 구조 walk를 재사용해 metadata와 novelty를 검사한다. `pipeline_omp_context_active_rpc.go` receipt, `pipeline_omp_context_active_process.go` identity, pin agreement 및 fixture도 같은 계약으로 갱신한다. provider 비용 관측이 불완전하면 fail-closed이며, probe 전용 checkpoint 호환성은 T0-C의 좁은 profile 밖으로 자동 확장하지 않는다.
...
## T0 circuit break and required compatibility amendment

T0 failed twice at the same checkpoint boundary. The probe-only OMP pin is restored to 17.2.7; no v2 measurement or refusal row is inferred from this partial run. Before another live run, revise/review the per-method checkpoint contract: distinguish a new authenticated method attempt from an ID replay; bound attempts by the verified chain; keep post-before-pre, replayed IDs, provider activity across the barrier and unverified authority rejected; record attempt-level outcomes without claiming complete billing. This is a protocol compatibility design task, not permission to remove the current ordering guard. T1–T7 remain blocked until that amendment and the probe evidence satisfy the gate.

## T0-C bounded checkpoint recovery (after amendment review)

- Add one internal checkpoint state machine used by manualCompact. Default limit stays one; the probe limit of two derives from the measured 18.1.13 digest and B overlay. Track request-ID replay, post/response phase and received-event counts. Before acknowledging a repeated pre, revalidate the initial transcript using the shared structural walker with a strict page-response reader. While each page is pending, anything except its exact successful response fails the transaction; provider/checkpoint/early compact frames cannot be silently consumed by protocol.call(..., false). Record authenticated rejected checkpoints without ACK. Never restart the deadline or infer provider-attempt costs.
- Extend the stateful fake OMP fixture, not only pure boolean tests: hold post/response frames until confirmations arrive, answer get_messages_page while the checkpoint waits, and inject reordered/replayed/changed-proof variants. Confirm the current implementation fails the legitimate two-pre fixture before changing admission.
- Update `pipeline_omp_context_active_lifecycle.go`, probe collector/record validation, evaluator/setup identity plumbing and fixtures. Production nil-probe behavior remains unchanged. AC-013 and existing managed RPC regressions must pass without providers, followed by security verify and one explicitly resumed live probe. T1–T7 remain gated on the resulting observation; this recovery does not reset the floor or treat partial data as measured.
