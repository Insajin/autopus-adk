...
Then `.agents/rules/autopus/lore-commit.md`의 첫 줄은 `---`이고 frontmatter는 `description: Lore commit format rules for structured, traceable commit messages`를 담는다.
And 그 frontmatter는 `category:`와 `name:`을 담지 않으며, `.agents/rules/autopus/branding.md`는 합성된 `description: Autopus Branding` 한 줄짜리 frontmatter를 얻고 본문 첫 줄은 여전히 `# Autopus Branding`이다.
And `.agents/rules/autopus/` 하위 14개 파일 각각은 frontmatter 이후 공백이 아닌 줄을 최소 1줄 갖고 `description` 또는 trigger 키를 가져 omp 세션에 도달하며, `.agents/rules/` 최상단에는 omp가 만든 파일이 하나도 없다.
...
And 사용자가 `.agents/rules/my-own-rule.md`와 `.agents/rules/autopus/user-extra.md`를 직접 만들었다.
When omp 어댑터가 `Generate` 후 `Clean`을 수행한다.
Then omp manifest는 `.agents/rules/autopus/` 14개, `.omp/agents/` 16개, `.omp/config.yml` 경로를 모두 담는다.
...
And `Clean` 이후 사용자가 만든 `.agents/rules/my-own-rule.md`와 `.agents/rules/autopus/user-extra.md`가 그대로 남는다.
...
Then `.agents/rules/autopus/` 하위 `.md` 파일 수는 정확히 14이고 파일명 집합은 `content/rules/`와 같다.
...
When `omp --mode rpc --no-session` 세션을 열고 시작 시 방출되는 `available_commands_update` 프레임과 `/dump`가 반환하는 시스템 프롬프트를 수집한다. `/context`는 토큰 사용량만 반환하고 `omp read rule://`는 세션 발견을 반영하지 않으므로 증거로 쓰지 않는다.
Then 규칙 14종이 모두 세션에 도달한다. `<domain-rules>` 목록 이름과 TTSR 등록 로그 이름의 합집합이 `content/rules/` 파일명에서 `.md`를 제거한 집합과 같고, 두 경로에 동시에 등록된 규칙은 없다.
...
And `auto init` 이후 `.gitignore`는 `.agents/rules/autopus/`, `.omp/agents/`, `.omp/config.yml` 줄을 담고 `/.omp/` 줄을 담지 않는다.
And `git status --porcelain`이 omp 생성 경로를 미추적 변경으로 보고하지 않는다.
And 그 `.gitignore` 적용 상태에서 omp 세션이 규칙 14종을 여전히 발견한다.
...
Then 방출된 `doc-storage` 규칙 본문은 `.claude/rules/` 대신 `.agents/rules/autopus/`를 담는다.
And 방출된 본문의 스킬 참조는 `.agents/skills/<name>/SKILL.md` 형태이며 `.agents/skills/<name>.md` 형태가 하나도 남지 않는다.
And branding 규칙 참조 문자열은 `.agents/rules/autopus/branding.md`이며 `content/rules/branding.md`가 아니다.
...
Then `.omp/rules/my-rule.md`와 `.omp/RULES.md`와 `.omp/` 디렉터리가 그대로 남고, 사용자가 편집한 `.omp/agents/executor.md`는 삭제 전에 백업 디렉터리로 보존된다.
...
And omp가 만들고 사용자가 건드리지 않은 `.omp/agents/planner.md`는 백업 없이 제거되며, `autopus.yaml`을 읽을 수 없으면 omp가 단독 소유를 증명할 수 없는 `.agents/skills/`는 건드리지 않는다.
...
And omp가 계속 소유하는 `.agents/rules/autopus/` 14개와 `.omp/agents/` 16개는 정상적으로 제거된다.
...
- Must 시나리오는 concrete expected output 또는 explicit tolerance를 담으며 구조 확인만으로 닫지 않는다. S3와 S12는 정규화 전후 문자열을 직접 비교해 no-op 회귀를 잡고, S4·S11·S13은 바이트 동일성과 백업 존재라는 결정적 오라클을 쓰며, S6·S9·S10·S11은 집합 동등성과 정확한 개수를, S7은 라이브 omp 세션의 실제 발견 결과를 예상 값으로 고정한다.
- 수치 허용 오차가 필요한 항목은 없다. 모든 기대값은 정확 일치다.

## Definition of Done

- [x] S1 부터 S15, E1 부터 E6 전부 통과 — 오라클 테스트와 2026-08-01 라이브 재실측(S7 규칙 14/14)으로 확인
