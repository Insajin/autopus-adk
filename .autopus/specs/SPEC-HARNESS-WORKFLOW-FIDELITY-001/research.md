...
6. ⭐⭐**END-TO-END BLOCKER 발견(CRITICAL, release 차단)**: executor fan-out e2e가 substrate-level 결함을 드러냄 — **Workflow 런타임 `isolation:'worktree'`가 만든 worktree(`.claude/worktrees/wf_*`)는 절대 merge되지 않는다**(변경 있으면 auto-remove도 안 됨, e2e가 2개 un-merged worktree 잔존으로 실증). 그런데 디스패처 계약(`harness-workflow.md` `### Segmented Dispatch Contract`: launch A → `auto workflow gate` → launch B → hygiene)에 **worktree merge 단계가 없다**. 문서는 "worktree reclaim은 Go runtime(`pkg/pipeline`) 소유"라 하나, `pkg/pipeline.WorktreeManager`는 legacy subagent 파이프라인용 **별개** worktree 메커니즘이고 route_team JS/디스패처는 이를 호출하지 않으며(`grep .claude/worktrees pkg/ internal/` → 소비처 0, route_team gate path는 WorktreeManager 미참조), Workflow-런타임 worktree와 연결되지 않는다. **결과**: route_team 종단은 plan✅ → executor가 N개 격리 worktree에 구현✅ → 하지만 그 변경이 orphan → `auto workflow gate`는 무변경 main tree를 build/test(vacuous pass) → segment B는 빈 tree를 annotate/review. **즉 executor 산출이 버려져 route_team이 종단 비기능**. 이는 FIDELITY-001 non-goal("worktree merge/coordination = Go/디스패처 소유, JS 이전 금지")이 전제한 Go merge 레이어가 **실재하지 않음**을 의미 — FIDELITY-001은 scope 내 정합(JS는 merge 안 함이 옳음)이나, 전체 substrate는 merge 레이어 부재로 default-on/release 불가. **release HOLD 권고 + 후속 SPEC 필요**: 디스패처(또는 신규 Go 단계)가 segment A 후 `.claude/worktrees/wf_<runid>-*`를 workingDir로 merge(충돌/file-ownership 처리 포함)한 뒤 gate 실행하도록 배선. 이것이 FIDELITY-001 Completion Debt "실 multi-agent real-LLM 종단 실행"의 구체적 blocker다.
