...
별도의 짧은 step이 K1 GitHub Environment secret을 0700 credential directory의 0600 파일로 만들고, 다음 step이 private/public/fingerprint tuple을 확인합니다. GoReleaser에는 파일 경로만 전달하며 `if: always()` cleanup이 제거 실패를 숨기지 않습니다. release run `29588526312`에서 Environment secret, checked-in K1 public key, full fingerprint의 exact pair와 cleanup이 통과했습니다. K2는 이 경로에 들어가지 않습니다.
...
v0.50.73 release는 annotated tag object `b1ebab0af82536f8a4bc1ed93f31f82f6c53d008`와 source commit `334b297f05942accbecdfa15b54e38e005c82f2d`에서 게시됐습니다. live `checksums.txt` SHA-256은 `a30e0893f1565919e9e90dd7e1f2b19e5487024b0373f66de56729e1d747e7d1`, `checksums.txt.signatures` SHA-256은 `a1b1643f78995fea5b773d8e213d4e33e1d71ff98ec142c14a6361a4e20cccb3`이며 OpenSSL과 exported Go verifier가 K1 signature를 검증하고 tampered checksum을 거부했습니다. 이어진 2026-07-18 v0.50.74 A5 하드닝 릴리스는 source commit `b27252cb1148192a8ae1a95195c50e5f221453a4`, tree `6c3790ee668b2d1c9f2f44a272144dd1106507d9`, annotated tag object `c79f133f0108bf3f07cee0162c1abeecf9d379d1`에서 변경 불가능한 최종 릴리스로 게시됐습니다. Release 실행 `29640813340`의 모든 gate와 정확한 source pin, K1 pair, A4→A5 public-key lineage, credential cleanup이 통과했습니다. 11개 자산의 GitHub 서버 digest를 대조한 결과 `checksums.txt` SHA-256은 `48c79e1fb47444aa83909794cd041bdfed18bf263bf5c0209578540382824ad4`, `checksums.txt.signatures` SHA-256은 `79c0afda3023a8f270e5386b88d87f7f219550f685c3fab8afe06cf5d70d1dc4`였고, 활성 K1 서명·변조 거부·cosign OIDC identity를 독립 검증했습니다. Darwin amd64·arm64 아카이브 SHA-256은 `aeb9d048579c77ab17f4a4ec3a1160778d16c627747c5af5f341e664e1417cb0`와 `bc90e594c91de61dabc2982f60249b638d448fa3f6643004fe6d45cdd0cc5eab`이며 두 바이너리의 Developer ID Team `GP2PFA2PUV`, hardened runtime, secure timestamp, notarized designated requirement와 `0755` 권한을 확인했습니다. exact-tag manifest verifier는 A0 receipt record SHA-256 `84ee9403223aabd1f60e5e55e79a5c7d6b2c764bc594435cbf7c4e997e2ce475`의 바이트 연속성, amd64 manifest `5b4381d3f2180b19c0da9d419ebc8452b9ba04c73c8d0921c2a74c09ab38b85c`와 arm64 manifest `62a9f78302ee000c16c1c73669282e955fc3abc82f850ff4a77d0e04069f4aed`의 서명·artifact binding도 검증했습니다. Homebrew commit `9e3b9b4076b47b85218b14632c79a3d796e6769c`는 Cask blob만 `ceed648bfece4555e8310b6e894fedc847520960`으로 갱신하고 Formula blob `4ebc6c38925002dec00759823d4dd847a499818a`는 유지했습니다.
...
Stage 2는 이 순서를 지킨 뒤 진행했습니다. POSIX installer는 SHA-256으로 고정한 raw-main verifier sidecar를 먼저 검증하고 source한 다음 V1 envelope를 확인합니다. Windows installer는 ASCII-safe PS5.1/7 공통 구현에서 SPKI→ECS1과 DER→P1363 변환 후 CNG로 검증합니다. CI run `29618589360`에서 macOS stock LibreSSL과 Windows PowerShell 5.1·7을 실제 실행했습니다.
...
`install.sh`와 `install.ps1`은 현재 GitHub raw main에서 직접 실행하도록 안내합니다. 신뢰된 설치기 바이트가 유지되는 동안 release assets만 교체한 공격자는 스크립트에 고정된 K1·K2 pin을 바꿀 수 없으므로 envelope 검증에 실패합니다. 그러나 repository main이나 raw-main 전달 경로를 장악한 공격자는 공개키·검증 코드·다운로드 대상을 바꿀 수 있어 release assets를 별도로 장악할 필요가 없습니다.
...
| repository main 또는 raw-main 전달 침해 | 명시적 threat-model 한계로 기록; 독립 bootstrap은 후속 작업 |
...
|---|---|---|
| V1 envelope parser·Go verifier | T1, T4 / S1~S6 | completed |
| multi-key producer·workflow preflight | T2, T3 / S7~S9 | completed |
| K1·K2 local ceremony·public pin | T1, T6 / S5, S8 | completed |
| K1 Environment·v0.50.73 live asset | T6 / S10 | completed |
| POSIX installer | T7 / S11 | completed |
| Windows PS5.1/7 installer | T8 / S12 | completed |
| 정상 UX·한계 문서 | T9 / S13 | completed |
...
| `install.sh`, `install.ps1` | Stage 2 consumer | signature → checksum → extract/install 순서와 floor·정상 UX oracle 확인 |
| `scripts/release-signing/verify-checksums-v1.sh` | Stage 2 POSIX verifier | strict wire/DER/SPKI/expiry와 pinned helper digest 확인 |
| `scripts/release-signing/tests/fixtures/v0.50.73/` | live release receipt | release/source/tag와 checksum/envelope SHA-256 확인 |
...
- **Completed scope**: V1 multi-sign envelope, deterministic producer, fresh K1·offline-next K2 public pin, K1 workflow key-file preflight, Go self-update verifier, API 호환, v0.50.73 live gate, POSIX·Windows installer, 문서 동기화.
- **Do not expand here**: K2 online signing activation, Homebrew/Apple/companion 변경, 독립 installer origin, SLSA/Rekor.
...
- **Completion status**: 독립 Stage 2 review와 cross-runtime CI가 P0/P1/P2 0으로 수렴했으며 전체 SPEC verdict는 APPROVE입니다.
...
- Q-COMP-08 | status: PASS | attempt: 3 | files: spec.md, plan.md, research.md, acceptance.md | reason: Environment/live/Stage 2 실제 run 증거를 S10~S13과 연결하고 Completion Debt 0을 확인함
...
| K1 GitHub Environment secret | RESOLVED | fresh K1 설정과 exact pair preflight |
| v0.50.73 live asset | RESOLVED | 실제 release와 독립 검증 |
| POSIX installer | RESOLVED | live gate 뒤 V1 parser·Ubuntu/macOS oracle PASS |
| Windows installer | RESOLVED | PS5.1/PS7 CNG oracle PASS |
| 세 소비자 문서 동기화 | RESOLVED | Stage 2 뒤 README/docs/CHANGELOG 반영 |

Completion Debt는 없습니다. SPEC 상태는 `completed`, review verdict는 `APPROVE`입니다. 운영 한계로 encrypted local backup과 Keychain 기반 recovery는 검증했지만, 별도의 off-device 독립 매체 보관은 현 환경에서 확인하지 못했습니다. repository main 또는 raw-main 전달 경로 침해와 함께 이 제한을 완료 범위 밖의 잔여 위험으로 계속 명시합니다.
