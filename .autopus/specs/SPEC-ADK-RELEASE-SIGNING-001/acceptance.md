...
### v0.50.76 A6 계보 재수렴 (PENDING LIVE RELEASE)
...
`v0.50.75` annotated tag는 source commit `70b9feff75fa75da330fa4bc93b59507924c8447`에 고정되어 있습니다. Release 실행 `29680368022`는 CI lint에서 Codex 훅의 미확인 `Close()` 오류 2건과 미사용 helper 1건을 탐지해 release job 전에 중단됐고, GitHub Release 자산이나 Homebrew 변경을 만들지 않았습니다. 실패 태그는 이동·재사용하지 않고 감사 이력으로 보존합니다.

Homebrew 전이는 A5 Cask blob `ceed648bfece4555e8310b6e894fedc847520960`을 prior CAS pin으로 사용하고 Formula blob `4ebc6c38925002dec00759823d4dd847a499818a`를 계속 동결합니다. 이 절은 코드·계보 준비 상태만 기록하며, `v0.50.76` 태그와 보호 환경 source pin, 실제 서명·공증·immutable release, Cask 게시의 라이브 증거는 릴리스 성공 후 별도로 동기화해야 합니다.
