...
- [ ] **T12 — 통합·sync 증거**: 구현 worktree의 `go test ./...`, actual RPC live gate, strict SPEC validation, `git status`, tracked-ignored hygiene 증거는 green이다. 다만 exact integrated commit/HEAD 재검증과 lifecycle sync 승격은 별도 commit 승인 뒤 수행한다.
