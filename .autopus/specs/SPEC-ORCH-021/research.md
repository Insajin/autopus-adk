...
## Contract Correction (2026-07-17)

이 문서의 Revision 1~3은 2026-05-30 당시 검토 이력으로 보존한다. 사용자가 2026-07-17에 실행 계약을 더 좁게 확정했다: **pane을 여는 데 실패한 경우가 아니면 subprocess를 사용하지 않는다.** 따라서 현재 normative 해석은 다음과 같다.

- 단일 provider는 `SplitPane`이 비어 있지 않은 pane ID를 반환한 순간 pane transport가 commit된다.
- subprocess fallback은 명시적 `--subprocess`, plain/nil/mux-unavailable 선택, 또는 commit 전 `SplitPane` 실패에만 허용된다.
- commit 후 provider launch command/build/send/enter, ready, prompt send/enter, completion, collection 실패는 pane-path failure/timeout으로 끝난다. cleanup-only 실패는 pane-path warning/receipt가 될 수 있다. 어느 경우에도 subprocess 호출 수는 0이어야 한다.
- multi-provider split은 transactional provisioning이다. 일부 pane만 만들어진 뒤 다음 split이 실패하면 provider CLI launch 전 partial pane을 cleanup한 뒤 subprocess fallback을 허용한다. 모든 필수 split이 성공한 순간 transaction이 commit되며, 그 뒤에는 fallback하지 않는다.
- debate/pipeline judge도 독립적으로 pane을 먼저 provision한다. judge 자신의 split 실패만 subprocess를 허용하며, judge pane ID commit 후 실패는 subprocess로 전환하지 않는다.
- hook→monitor/poll 및 response-file→scrollback은 committed pane 안의 detector/collection degrade이므로 허용된다. 이는 backend 전환이 아니다.
...
- "`-p`=API" 전제는 provider마다 다를 수 있다. 예컨대 `agy --print`는 gemini 로그인(구독) 세션을 사용할 가능성이 있어 항상 별도 API 키를 요구하지 않을 수 있다(코드만으로 단정 불가 — confidence: low). SPEC은 호출 형태(argv 정합)를 generic하게 처리하고, "구독 세션은 인터랙티브가 기본"이라는 원칙은 유지하되 provider별 `-p` 가용성 차이를 강제 가정하지 않는다. 이 불확실성은 **pre-commit provisioning fallback**(REQ-005)과 그 fallback까지 실패할 때의 actionable 에러(REQ-013)에서만 흡수한다.
...
- 함의: (1) pane은 "fallback 회피용 default"가 아니라 다수 유저의 primary path다. (2) `-p` fallback은 commit 전 pane provisioning 실패에만 쓰는 best-effort이며 구독-only 유저에겐 미가용일 수 있으므로, provisioning+`-p` 동시 실패 시 raw API 에러가 아니라 actionable 에러가 필요하다(REQ-005/REQ-013). (3) pane ID가 확보된 뒤에는 실패해도 subprocess로 우회하지 않는 것이 정책/인증 경계를 보존한다. (4) 진짜 목표는 default flip이 아니라 **pane의 안정성과 transport stickiness**다 — `ca7f026`이 드러낸 약점을 정면으로 닫는다(REQ-009~REQ-012).
- confidence: high(코드상 `-p`=API 경로 확인). 구독 세션이 인터랙티브 CLI로만 동작한다는 점은 provider별 정책에 의존하므로 메커니즘 설계는 그 가정을 강제하지 않는다. `-p` 미가용은 pre-commit fallback의 일반적 실패 케이스로만 다룬다(actionable 에러).
...
- **User-visible outcome**: 구독 세션만 보유한 유저가 cmux/tmux 가능한 터미널에서 `/auto idea`, `/auto spec review`, `auto orchestra run`을 실행하면 provider가 `-p`(API) 없이 **로그인된 인터랙티브 pane 세션으로 안정적으로 멀티프로바이더 실행**된다(세션 ready 후 전송, bounded 완료 감지, hang 없음, hook 미설치여도 in-pane degrade하여 동작). plain/CI, 명시적 `--subprocess`, mux unavailable, 또는 pane split provisioning 실패면 `-p` subprocess를 시도할 수 있다. 비어 있지 않은 pane ID는 accompanying error가 있어도 commit이며, 이후 실행/완료/수집 실패는 pane-path failure/timeout, cleanup-only 실패는 pane warning/receipt로 귀속되고 subprocess로 전환하지 않는다. pre-commit provisioning fallback도 미가용이면 raw API 에러가 아니라 **actionable 에러**로 끝내고 실행 경로(또는 둘 다 실패)를 기록한다.
- **Mandatory requirements**: brainstorm pane 기본값(REQ-001); spec review structured pane-first(REQ-002); orchestra run structured pane-first(REQ-003); structured용 진짜 pane `ExecutionBackend` 존재(REQ-004); **commit 전 provisioning 실패에서만** best-effort subprocess fallback + commit 후 subprocess 금지 + judge pane-first + 실패 시 actionable 에러 + 경로 기록(REQ-005); spec review에 terminal 감지 주입(REQ-006); 단일 공유 pane-capability 술어를 양쪽 선택 사이트가 소비(REQ-007); pane screen 출력 정제(REQ-008); **pane 신뢰성 그룹** — 세션 ready 전 미전송(REQ-009), 완료 감지 monitor→poll bounded in-pane 전환(REQ-010), timeout 시 결정적 실패 no-hang/no-subprocess(REQ-011), hook 미설치 in-pane degrade(REQ-012), pane provisioning+`-p` 동시 실패 시 actionable 에러(REQ-013); **provider 호출 정합성 그룹** — subprocess argv 프롬프트 형태 정합(REQ-014, gemini `--print` 값/codex `exec`+`--output-schema`), pane argv 대화형 형태(REQ-015), 구조화 리뷰 provider 참여 정합 + codex `--output-schema`(REQ-016).
- **Accepted assumptions**: (a) sanitize된 pane scrollback에 provider가 JSON을 출력하면 `extractJSON`이 추출 가능하다(파서가 prose-tolerant임을 코드로 확인). (b) CC21 Monitor 경로가 활성일 때 pane 완료 지연이 `ca7f026` 당시보다 완화됐다(`waitForCompletion` 구조로 확인). (c) 기존 interactive 헬퍼(launch/세션ready/완료감지/수집)를 단일 provider 단위로 묶어 재사용할 수 있다. (d) 구독-only 유저에게 `-p`는 미가용일 수 있으므로 pre-commit provisioning fallback은 best-effort이고, 미가용은 정상적 실패 케이스로 actionable 에러를 낸다. (e) hook IPC가 가장 안정적이나 설치 의존이므로 hook 미설치 시 monitor→poll degrade가 허용 가능한 신뢰성을 준다(`resolveCompletionDetector` 구조로 확인). 이 degrade는 pane 내부 detector 교체이며 subprocess 전환이 아니다. If wrong: hook 없이 poll만으로 지연이 과도하면 후속 튜닝이 필요(Evolution). (f) `-p`가 API를 요구하는 정도는 provider별로 다를 수 있다(예: `agy --print`는 gemini 로그인 세션 사용 가능성 — confidence: low, 코드만으로 단정 불가). SPEC은 argv 정합을 generic하게 다루고 provider별 `-p` 가용성을 강제 가정하지 않으며, 불확실성은 provisioning-only fallback(REQ-005)+actionable 에러(REQ-013)로 흡수한다. (g) provider 설정 SoT는 `autopus-adk/autopus.yaml`이고 `orchestra_helpers.go buildProviderConfigs`는 config 부재 시 fallback이므로 정정은 양쪽에 적용한다(라이브 확인: gemini는 config에 `args` 없어 fallback이 버그 형태를 지배).
...
- **Completion evidence**: truth table oracle S1~S9 + commit-lock oracle S6b/S6c + pane 신뢰성 S10~S14 + provider 호출 정합성 argv oracle S15~S20 + judge pane-first S21 통과. transport-lock evidence는 non-empty-ID+error commit, post-commit launch/ready/prompt/completion/collection/cleanup marker 0, cleanup retry/untrack, partial split cleanup-before-fallback, judge pane-first + empty-ID fallback을 포함한다. `go test ./pkg/orchestra -count=1`(81.886s), `go test ./internal/cli ./pkg/terminal ./pkg/config -count=1`(73.248s/1.216s/0.940s), `go vet ./pkg/orchestra ./internal/cli ./pkg/terminal ./pkg/config`, `go build ./...`, independent convergence review(`remaining_findings=none`)가 모두 통과했다. 운영 검증은 `auto doctor` provider-smoke로 보강할 수 있다(환경 의존).

## Visual Planning Brief

`plan.md`의 `## Visual Planning Brief` Mermaid flowchart 참조. 흐름: 구독 세션→pane(primary), plain/nil/명시적 subprocess→`-p`, pane `SplitPane` provisioning 실패→`-p` best-effort→둘 다 실패 시 actionable error다. pane ID commit 뒤에는 세션 ready 게이트(REQ-009)→bounded 완료 감지(monitor→poll, REQ-010)→timeout 시 결정적 pane 실패(REQ-011)로 흐르고 subprocess로 되돌아가는 간선은 없다. `SelectBackend`는 상위 분기, `Execute` 내부 신뢰성 스테이지는 하위 분기이며 모든 종착에서 실행 경로를 기록한다. judge도 자체 split-first/commit-lock을 적용한다.
...
- `ca7f026`은 brainstorm pane에서 SignalDetector 타임아웃→poll fallback 지연을 이유로 subprocess 기본값을 택했다. 그러나 (1) provider CLI들의 `-p` 제한/API 과금 이전이라는 새 제약이 비용/가용성 측면에서 더 크고, (2) `cc21_monitor.go`의 Monitor-우선/poll-fallback(`waitForCompletion`)과 `monitor-patterns.md`가 그 지연을 완화한다. trade-off: pane 기본값은 평균 지연이 다소 늘 수 있고 commit 후 실패는 subprocess로 복구하지 않지만, 정책/인증 경계를 지키고 bounded pane failure로 완료성을 보장한다. subprocess 복구는 split provisioning 전 경계에만 보존된다. confidence: medium(지연 개선은 코드 구조로 확인했으나 실측 벤치마크는 이 SPEC 범위 밖).

### Pane 신뢰성 설계 — 기존 메커니즘 재사용, 재발명 금지 (REQ-009~REQ-013)

- 진짜 목표는 default flip이 아니라 pane 안정성이므로, 새 `Execute`는 오케스트레이션 레벨(`RunInteractivePaneOrchestra`)이 이미 검증한 신뢰성 순서를 **단일 provider 단위로 압축 재사용**한다: split → **non-empty pane ID에서 transport commit(error 동반 여부 무관)** → launch → `pollUntilSessionReady`(ready 전 미전송, REQ-009) → send → `waitForCompletion`(monitor→poll, bounded, REQ-010) → 미완료 시 결정적 pane `TimedOut`(REQ-011) → `ReadScreen`+sanitize. commit 뒤 실행 실패는 pane failure/timeout, cleanup-only 실패는 pane warning/receipt로 귀속되고 subprocess 호출은 없다.
...
- fallback 의미(REQ-005/REQ-013): 구독-only 유저에게 `-p`는 인증 실패할 수 있다. 따라서 **pane split provisioning이 commit 전에 실패했을 때만** `-p`를 best-effort로 시도한다(성공 시 `executed=subprocess` 기록). provisioning AND `-p` 둘 다 실패면 양 원인 + 복구 지시("ensure a logged-in cmux/tmux CLI session")를 담은 actionable 에러를 낸다. non-empty pane ID commit 뒤에는 `-p`를 호출하지 않는다. multi-pane은 provider CLI launch 전 partial split을 cleanup한 뒤에만 fallback하고, judge도 자체 split에 같은 규칙을 적용한다.
...
- 새 메커니즘 작성 금지: 위 모든 것은 `interactive_session_ready.go`/`cc21_monitor.go`/`completion_file_ipc.go`/`screen_sanitizer.go`의 기존 심볼 재사용이다. 새 backend 파일은 "단일 provider 래퍼 + provisioning-only fallback + transport commit lock + 에러 구성"만 추가한다.

## Semantic Invariant Inventory

| ID | source clause | invariant type | affected outputs | acceptance IDs |
|----|---------------|----------------|------------------|----------------|
| INV-001 | "interactive pane mode is the DEFAULT; subprocess is explicit/non-capable/pre-commit provisioning fallback" | backend-selection keyed by the shared predicate `paneCapable(term, subprocessMode)` (incl. non-nil `"plain"` → subprocess, F-001); consumed by both `runner.go` guard and `SelectBackend` | pane-vs-subprocess mode decision (predicate result) consumed by both selection sites | S1, S2, S3, S4, S5, S7, S9 |
...
| INV-004 | "commit 전 SplitPane provisioning 실패만 `-p` best-effort; non-empty pane ID(+error 포함) commit 후 subprocess 금지; judge도 동일" | transport state machine: pre-commit fallback allowed, post-commit sticky pane path; transactional partial-split cleanup-before-fallback | fallback response backend id; post-commit subprocess call count 0; judge split-first/empty-ID fallback ordering | S6, S6b, S6c, S21 |
| INV-005 | "record which path ran" | observability of executed backend including failed/timed-out committed pane path and cleanup-only pane warning/receipt | recorded backend id/receipt per response, failure, or cleanup warning | S2, S6, S6b |
| INV-006 | "pane screen-scraping handles untrusted provider output → must redact/sanitize" | sanitization before parse | sanitized backend output (no ANSI/banner) | S8a |
...
| INV-008 | "세션 ready 전 prompt 미전송" (입력 유실 방지) | ordering invariant: `isSessionReady` true 이후에만 send, `startupTimeoutFor` bounded; post-commit timeout stays pane-only | prompt-send timing; unready→failed (no send); subprocess calls 0 | S10, S6b |
| INV-009 | "완료 감지 hook/monitor→poll 계층 + bounded timeout" | layered in-pane detector reuse (`waitForCompletion`), `MonitorTimeout` bound; no backend switch | detector path (event-driven→poll) within timeout; subprocess calls 0 | S11 |
| INV-010 | "완료 timeout 시 결정적 실패 결과(hang/garbage/subprocess 우회 아님)" | bounded termination → deterministic timed-out pane ProviderResponse | `Execute` returns within timeout, `TimedOut`/failed set, subprocess calls 0 | S12, S6b |
| INV-011 | "hook 미설치 시 monitor→poll로 graceful in-pane degrade" | conditional detector resolution (`resolveCompletionDetector`, `HookMode` off → non-hook), backend unchanged | resolved detector is non-hook; execution still completes or times out in pane | S13 |
| INV-012 | "pane provisioning 실패+`-p` 미가용 시 actionable 에러" (구독 현실 비은폐) | pre-commit both-failed → actionable error (not raw API error); unreachable post-commit | error string names both causes + recovery instruction | S14 |
...
| pre-commit pane provisioning 실패만 best-effort `-p` fallback + actionable 에러 + 경로 기록 | Primary SPEC (T5,T6,T9, REQ-005/REQ-013) | covered |
| pane transport commit 후 subprocess 금지 + partial-split transactional cleanup + judge pane-first | Primary SPEC (T5,T9b, REQ-005) | covered |
...
| pane 신뢰성: 세션 ready 게이팅 / bounded 완료 감지 / no-hang / hook in-pane degrade | Primary SPEC (T8a,T8b, REQ-009~REQ-012) | covered |
...
|------|----------------------|-------------------|
| Provider-API execution backend (call provider HTTP/SDK APIs instead of CLI) | 사용자의 "moving to API billing" 전제에 대응하는 전략적 미래 작업이나, Outcome Lock(=pane-first + provisioning-only subprocess fallback + post-commit transport lock)을 닫는 데 필요하지 않다. 별도 인증/과금/레이트리밋 경계가 필요. | 사용자가 명시적으로 API 백엔드를 요청할 때 |
...
|----------|--------|------------------|
| none | Primary SPEC가 단일 cohesive change(세 structured 진입점의 backend 기본값 역전 + 진짜 pane backend + pane 신뢰성 경화 + provisioning-only fallback + post-commit transport lock)로 Outcome Lock을 닫는다. 신뢰성 그룹(REQ-009~013)은 동일 pane backend의 내부 동작이라 분리하면 스캐폴드만 남으므로 같은 SPEC에 둔다. 독립 배포 경계·migration sequencing·보안 경계 분리 사유 없음. 태스크 ~14개/신규 소스 파일 ~4개 + 수정 6개로 25태스크·40파일 임계 미만이다. | None |
...
| provisioning-only `-p` fallback + actionable 에러 + post-commit transport lock (REQ-005/REQ-013) | `[NEW] pkg/orchestra/pane_fallback.go` (또는 backend 파일 내 함수) | planned addition |
| pane 신뢰성/commit-lock oracle tests (S6b/S6c/S10~S14/S21) | `[NEW] pkg/orchestra/pane_backend_reliability_test.go` | planned addition |

## Reviewer Brief

- **Intended scope**: 세 structured 진입점의 실행 backend 기본값을 pane-first로 역전 + 진짜 pane `ExecutionBackend` 추가 + **pane 신뢰성 경화(세션 ready 게이팅·bounded 완료 감지·no-hang·hook in-pane degrade, REQ-009~012)** + **commit 전 split provisioning-only `-p` fallback** + non-empty pane ID commit 후 subprocess 금지 + multi-pane transactional cleanup + judge pane-first + provisioning+`-p` 동시 실패 시 actionable 에러(REQ-005/REQ-013) + 실행 경로 기록 + spec review에 terminal 감지 주입. 신뢰성은 기존 메커니즘 재사용(재발명 없음).
...
- **Self-verified**: Traceability Matrix(REQ-001~016↔task↔AC↔INV), Semantic Invariant Inventory(INV-001~016), oracle/behavioral acceptance S1~S21(S6 provisioning fallback, S6b post-commit subprocess 0, S6c partial-split transaction, S8a/b/c parse, S9 술어 합의, S10 ready-게이팅, S11 monitor→poll in-pane, S12 no-hang/no-subprocess timeout, S13 hook in-pane degrade, S14 pre-commit both-failed actionable 에러, S21 judge pane-first), 신뢰성 메커니즘이 기존 심볼 재사용임을 Reference Discipline에서 확인, existing/[NEW] 경계.
- **Reviewer should focus on**: correctness(공유 술어 `paneCapable`의 7행 정확성 + F-001 plain + F-002 양쪽 소비), transport state machine(non-empty pane ID commit 경계, post-commit fault branches의 subprocess 호출 수 0, multi-pane partial cleanup before fallback, judge split-first), pane 신뢰성(세션 ready 전 미전송, `waitForCompletion` 재사용으로 monitor→poll bounded, timeout 시 결정적 no-hang/no-subprocess, hook 미설치 degrade가 기존 `resolveCompletionDetector` 동작과 일치하는가), 재발명 금지(새 backend가 새 감지기를 만들지 않고 기존 심볼만 재사용하는가), fallback 의미(pre-commit provisioning-only `-p` + both-failed actionable 에러가 raw API 에러를 숨기지 않는가), regression risk(legacy `RunInteractivePaneOrchestra` 멀티라운드 경로·plan/review/secure 미변경, 새 backend/fallback 파일 300줄 한계), Completion Debt(없음 — 반증 시에만).
...
사용자 우선순위 재구성(subscription-first, pane 안정성 = 진짜 목표) 반영. 기존 8 REQ 유지, 신뢰성 그룹 추가. 이 절은 당시 closure 기록이며, fallback transition 경계는 아래 Revision 4가 supersede한다.
...
- REQ-005 정정 + REQ-013 (신규, historical) | correctness | 당시 fallback을 "무조건 -p"에서 "best-effort -p → 실패 시 actionable 에러(구독 현실 비은폐)"로 정정했다. Revision 4는 이를 commit 전 split provisioning 실패로 더 좁힌다. | spec.md REQ-005 재작성·REQ-013·Matrix, plan.md T9·flowchart, acceptance.md S6 정정·S14, research.md INV-004 정정·INV-012·설계
- Visual Planning Brief (historical) | completeness | 당시 subscription→pane(primary)/API→-p(best-effort)/both-fail→actionable error를 반영했다. 현재 flowchart는 split commit 전 fallback과 post-commit terminal pane failure를 분리한다. | plan.md `## Visual Planning Brief`
...
## Revision 4 contract correction

사용자 명시("pane 여는 게 실패한 게 아니면 subprocess를 하지 않는 게 목표")를 normative transport state machine으로 반영했다. 기존 완료 이력은 보존하되 현재 REQ-005와 acceptance가 아래 경계를 우선한다.

- transport commit | correctness | 단일 provider `SplitPane`의 non-empty pane ID 반환을 commit point로 정의. launch command/build/send/enter, ready, prompt send/enter, completion, collection, cleanup 후속 실패에서는 subprocess 호출 수 0 | spec.md REQ-005, acceptance.md S6b, plan.md T5/T9b, research.md INV-004
- provisioning-only fallback | correctness | 명시적 subprocess·plain/nil/mux-unavailable·commit 전 split 실패만 subprocess 허용. provisioning과 subprocess가 모두 실패할 때만 actionable combined error | spec.md REQ-005/REQ-013, acceptance.md S6/S14, plan.md T9, research.md INV-004/INV-012
- transactional fan-out | correctness | multi-provider partial split 실패 시 provider CLI launch 전 partial panes cleanup을 완료하고 나서 fallback 허용 | spec.md REQ-005, acceptance.md S6c, plan.md T9/T9b, research.md INV-004
- judge pane-first | completeness | judge가 독립적으로 split을 먼저 시도하고 judge split 실패에서만 subprocess 허용; judge pane commit 후 subprocess 금지 | spec.md REQ-005, acceptance.md S21, plan.md T9b, research.md INV-004
- in-pane degrade 구분 | correctness | hook→monitor/poll과 response-file→scrollback은 backend fallback이 아닌 committed pane 내부 degrade로 명시 | spec.md REQ-005/REQ-010~012, acceptance.md S11/S13, plan.md T8b, research.md INV-009/INV-011
- historical completion correction | traceability | 2026-05-30 완료 기록을 지우지 않고 2026-07-17 계약 정정 및 S6b/S6c/S21 current Must oracle을 추가 | spec.md Completion Verdict, acceptance.md contract note, plan.md contract note, research.md Contract Correction
- implementation verification | evidence | focused transport-lock/S6/S12/S14/judge/partial-split/tracker tests GREEN: non-empty-ID+error commit, six early post-split marker-zero cases, completion/collection/cleanup marker zero, cleanup retry/untrack, partial cleanup-before-fallback, judge pane-first + empty-ID fallback. Full gates also PASS: `go test ./pkg/orchestra -count=1`(81.886s), `go test ./internal/cli ./pkg/terminal ./pkg/config -count=1`(73.248s/1.216s/0.940s), scoped `go vet`, `go build ./...`; independent convergence `remaining_findings=none` | focused + full regression/build/vet/review evidence; spec.md Completion Verdict
...
- Q-COMP-02 | status: PASS | attempt: 5 | files: spec.md, acceptance.md, plan.md, research.md | reason: 16 REQ 전부 task/AC/INV로 추적하며, Rev 4의 REQ-005 transport state machine을 T9/T9b↔S6/S6b/S6c/S14/S21↔INV-004/INV-012로 추가 매핑했다.
...
- Q-COMP-04 | status: PASS | attempt: 3 | files: research.md, spec.md | reason: Outcome Lock을 '구독 pane 우선 + provisioning-only fallback + post-commit subprocess 금지 + judge pane-first'로 정정하고 REQ-005/009~013 + S6/S6b/S6c/S10~S14/S21로 닫았다. Completion Debt는 None이다.
- Q-COMP-05 | status: PASS | attempt: 5 | files: research.md, spec.md, plan.md, acceptance.md | reason: post-commit 각 fault stage의 subprocess 호출 수 0(S6b), transactional partial split cleanup(S6c), judge split-first(S21)를 직접 관측 가능한 oracle로 추가했다. monitor→poll/hook degrade는 in-pane임을 S11/S13에서 단언한다.
- Q-COMP-06 | status: PASS | attempt: 4 | files: spec.md, research.md | reason: Reviewer Brief와 Traceability를 16 REQ 및 Rev 4 transport contract로 동기화했다.
