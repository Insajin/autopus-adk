...
S1과 S3~S9는 정책 자체와 fallback을 섞지 않도록 `C_FULL`을 사용한다. supervisor/orchestra에서
...
Given `C_FULL`과 Balanced/Ultra supervisor·orchestra의 declared effort=`not-applicable`이 주어진다
And Balanced Opus=`max`, Sonnet=`medium|high`, Haiku=`low`, Ultra worker=`low|medium|high|max|ultra|unknown|blank` declared effort가 주어진다
When canonical desired-profile resolver와 capability resolver를 순서대로 호출한다
Then Balanced supervisor/orchestra는 정확히 `gpt-5.6-sol+xhigh`이다
And Ultra supervisor/orchestra는 정확히 `gpt-5.6-sol+ultra`이다
...
And 모든 Ultra worker는 정확히 `gpt-5.6-sol+max`이다
...
And exact `ultra`가 아닌 custom preset은 supervisor/orchestra에서 Balanced profile을 사용한다
And custom preset으로 persistent agent를 생성할 때는 그 preset의 role tier mapping을 유지한다

### S3: fresh Balanced supervisor가 Sol+xhigh를 생성한다 (REQ-003)

Given `C_FULL`, `quality.default=balanced`, declared effort=`not-applicable`인 fresh Codex init이 주어진다
When `.codex/config.toml`을 생성한다
Then root는 `model="gpt-5.6-sol"`과 `model_reasoning_effort="xhigh"`를 포함한다
And fallback receipt는 발생하지 않는다

### S4: fresh Ultra supervisor가 Sol+ultra를 생성한다 (REQ-003)

Given `C_FULL`, `quality.default=ultra`, declared effort=`not-applicable`인 fresh Codex init이 주어진다
When `.codex/config.toml`을 생성한다
...
### S6: Ultra worker는 declared effort와 무관하게 Sol+max를 사용한다 (REQ-004)

Given `C_FULL`과 각 managed agent의 source tier 및 `low|medium|high|max|ultra|unknown|blank` declared effort가 주어진다
When Ultra config로 모든 managed agent 정의를 렌더한다
Then 각 agent는 정확히 `gpt-5.6-sol`과 `max`를 사용한다
...
When Ultra supervisor 또는 orchestra의 `gpt-5.6-sol+ultra`를 resolve한다
...
Given Ultra, fresh root declared effort=`not-applicable`, agent=Sonnet/`medium`, quality-managed provider가 주어진다
When `C_FULL`, `C_SOL_NO_ULTRA`, `C_VALID_MISSING`을 동일 resolver에 각각 적용한다
Then `C_FULL`에서 root=`Sol+ultra`, agent=`Sol+max`, Args/PaneArgs=`Sol+ultra`이다
And `C_SOL_NO_ULTRA`에서 root=`Sol+max`, agent=`Sol+max`, Args/PaneArgs=`Sol+max`이다
...
And persistent worker mode 변경에는 `auto quality set`, `auto update`, 새 Codex 세션이 필요하다고 명시한다
...
| S3, S4 | `TestGenerateConfig`, `TestGenerateConfig_UsesUltraQualityProfile`, `TestInitCmd_QualityBalancedSetsCodexProfile`, `TestInitCmd_QualityUltraSetsCodexProfile` |
| S5, S6, S7 | `TestTransformAgentForCodex_RendersQualityAwareProfiles`, `TestGenerateAgents_BalancedQualityUsesRoleEffort`, `TestGenerateAgents_UltraQualityUsesSolMax`, template parity tests |
...
| S10 | `TestGenerateConfig_PreservesUserModelValueLiteral`, `TestGenerateConfig_PreservesExistingCodexModelSettings`, `TestUpdate_PreservesUserCodexModelSettings`, `TestGenerateConfig_RuntimeDefaultStillPreservesUserModel` |
...
| S19 | `TestCodexQualityGuidanceDocumentsLoadedAgentBoundary`와 adaptive-quality/agent-pipeline source/template parity tests |
