...
- [x] T2: `templates/codex/config.toml.tmpl`과 Codex adapter merge를 갱신한다. fresh root는
  ...
  drift·quoted key·multiline TOML fixture를 분리 검증한다. 생성 헤더, manifest merge policy,
  matching checksum, exact managed tuple이 모두 확인된 legacy root만 `auto update`에서 `inherit`로
  이행하고, preview 무쓰기, doctor 경고, Codex 실패 시 policy 원복, workspace config rollback을
  회귀 테스트로 고정한다.
...
- [x] T4: canonical Codex orchestra provider에 `model_policy: quality`를 표시하고 config migration 및
  ...
  quality로 이행한다. v0.50.66이 자동으로 pinned 처리한 exact model-only signature도 raw supervisor
  policy가 없을 때만 quality로 복구한다. custom/pinned Binary·Args·PaneArgs와 `--` suffix를 slice
  equality로 보존하는 테스트를 먼저 작성한다.
...
- [x] T7: `acceptance.md`의 S1~S22 evidence map에 따라 focused tests, template parity,
...
없음. REQ-001~REQ-010과 S1~S22를 구현 및 hermetic test evidence로 모두 닫는다.
