...
| **claude trust-folder 프롬프트 (남은 블로커, 실환경 한정)** | 미신뢰 디렉토리에서 pane claude 세션 시작 | cmux pane의 claude가 **미신뢰 폴더에서 trust 다이얼로그("Is this a project you trust? 1.Yes 2.No")로 블록** → 세션 미시작 → SessionStart/Stop hook 미발화 → ready=false → subprocess fallback. `--dangerously-skip-permissions`는 trust를 스킵 못 함(권한만). 실 사용자 프로젝트는 이미 trusted(`~/.claude.json projects[dir].hasTrustDialogAccepted=true`)라 정상이나, 미신뢰/신규 dir에선 행. 해소: 오케스트레이터가 launch 전 WorkingDir를 `~/.claude.json`에 hasTrustDialogAccepted=true로 사전등록하거나, trust 프롬프트 자동수락(Enter) 전송. **검증 한계**: 샌드박스 테스트 환경(미신뢰 temp dir + TMPDIR=/tmp/claude-502 + ad-hoc settings + interactive-claude-in-pane)이 실 trusted-project + 실 cmux 세션을 충실히 재현 못 해, 다층 fix 후에도 end-to-end done-file 수집을 demonstrable하게 확인 못 함. unit/instrument로 각 fix는 검증됨. |
