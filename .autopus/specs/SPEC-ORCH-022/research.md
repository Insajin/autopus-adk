...
| **완료(Stop) done-file 미관측 — 남은 BLOCKING** | done-file IPC **완료** 수집 | ready는 hook-IPC로 해결됐으나, TRUSTED e2e에서 **Stop hook의 done-file이 140s timeout 내 미출현** → 완료는 여전히 screen-scrape 폴백으로 수집(review verdict는 정상 산출). 원인 후보: (a) 인터랙티브 review 응답이 140s보다 길어 done-file이 timeout 후 도착, (b) 인터랙티브 claude의 Stop hook 발화 타이밍이 collectResponse/cleanup보다 늦음. 해소: 충분한 timeout(예: 300s+)로 done-file 도착 재확인, 또는 Stop-hook-interactive 타이밍 조사(SessionStart는 발화 확인됨이므로 settings/env는 정상). Smoke 2a에서 `claude -p`의 Stop→done는 확인됨. |

go 단계 oracle(라우팅 truth-table, env-injection 안전성, hookCollectionEligible, detector 선택, bounded timeout)은 모두 통과한다. **2026-06-10 진행 요약**: TMPDIR/WorkingDir/SessionStart-ready/env-injection 4 fix로 **session-ready를 hook-IPC(done-file)로 결정론화 검증 완료**(TRUSTED 위치 e2e, ready 5s). trust 프롬프트는 조상-trust 기반이라 실 프로젝트(~/ 아래)에선 무이슈로 규명. 남은 BLOCKING은 **완료(Stop) done-file 수집**(현재 screen-scrape 폴백으로 review는 동작하나 done-file IPC 완료는 미확인) — 충분한 timeout 재검증 또는 Stop-hook-interactive 타이밍 조사 필요. 따라서 이 SPEC은 `implemented`이며 완료 done-file IPC 확인 전까지 `completed` 금지(false-complete 방지).
