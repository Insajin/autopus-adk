...
| ~~session-ready 게이트 화면 스크래핑 의존~~ **[ADDRESSED 2026-06-10]** | ready 결정론화 | SessionStart hook(`hook-{provider}-sessionstart.sh`, generateCompletionHooks 등록)이 ready 파일 작성, `waitForPaneReady`(interactive_poll.go)가 ready 파일+화면 패턴을 한 루프로 감시(파일 우선). claude/gemini/codex 모두 ready 스크립트 추가. |
| ~~TMPDIR 불일치: NewHookSession=os.TempDir()/autopus vs hook 스크립트 하드코딩 /tmp/autopus~~ **[FIXED 2026-06-10]** | hook 발화(dir-exist guard) | `SendSessionEnvToPane`이 `AUTOPUS_SESSION_DIR`(=os.TempDir 기반 실 dir) export, 6개 hook 스크립트가 `${AUTOPUS_SESSION_DIR:-/tmp/autopus/...}` 사용. TMPDIR≠/tmp(샌드박스) 환경에서 hook이 잘못된 dir 보고 no-op하던 것 해소. |
| ~~pane provider WorkingDir 미설정~~ **[FIXED 2026-06-10]** | pane claude가 hook 설치 settings.json 읽기 | spec_review_loop orchCfg에 `WorkingDir=os.Getwd()` 추가 → pane claude가 프로젝트로 cd → 프로젝트 `.claude/settings.json`의 hook 읽음. |
| ✅ **session-ready hook-IPC 검증 성공 (2026-06-10)** | — | **TRUSTED 위치(~/Documents 아래) 실 spec review e2e에서 5초만에 `claude-round0-ready` 파일이 오케스트레이터 감시 dir에 출현** → `waitForPaneReady`가 감지 → 화면 스크래핑 ready flakiness 완전 제거. routing→pane→hookSession→SessionStart hook→ready 파일→prompt 전송 체인 검증됨. review도 정상 완료(판정 REVISE, 발견 7, 체크리스트 23). |
| **claude trust = 조상 디렉토리 기반 (테스트환경 함정 규명)** | 미신뢰 위치(예: /tmp)에서 pane claude 세션 시작 | claude는 **trusted 조상 dir 아래면 신뢰**(`/Users/bitgapnam/Documents`=hasTrustDialogAccepted:true → 그 아래 모든 프로젝트 프롬프트 없음). `/tmp/*`는 조상 밖이라 trust 다이얼로그로 블록 → 세션 미시작. **실 사용자 프로젝트는 ~/ 아래(trusted 조상)라 프롬프트 없이 정상.** `~/.claude.json projects[dir].hasTrustDialogAccepted=true` 직접 pre-seed는 무효(claude가 조상 기준 판단 + live 세션 clobber race) → trust pre-seed 시도는 revert함. 미신뢰 위치 지원은 불필요(실 프로젝트 trusted)하며 별도 robustness 영역. |
| ~~완료(Stop) done-file 미관측 — 남은 BLOCKING~~ **[RESOLVED 2026-06-11]** | done-file IPC **완료** 수집 | **근본 원인 규명(타이밍 가설 둘 다 기각)**: 계측 e2e가 Stop hook 발화 시점에 `AUTOPUS_SESSION_DIR exists=no`임을 포착 — Stop hook은 정상 발화하고 `last_assistant_message`도 정상(인터랙티브/`-p` 동일)이나, **완료 detector가 `FileIPCDetector`가 아니라 `ScreenPollDetector`로 떨어져** 화면 렌더 응답을 done-file보다 먼저 감지→`Execute` 반환→`pane_backend.go`의 `defer hookSession.Cleanup()`이 세션 dir 삭제→직후 Stop hook이 발화하나 dir 없음→`[ -d SESSION_DIR ]` 가드로 no-op. 원인: `resolveCompletionDetector`(`pkg/orchestra/cc21_monitor.go`)가 FileIPCDetector를 `cfg.MonitorEnabled`(=`features.Monitor`) 뒤에 게이팅해, 모니터 기능 off 시 done-file floor가 비활성. **수정**: HookMode+hookSession이면 monitor flag/cap과 무관하게 `FileIPCDetector`를 full-budget(eventDriven=false) floor로 우선 선택 → `Execute`가 done-file까지 블록 → 세션 dir이 Stop hook 발화까지 생존. 부가: `hook-claude-stop.sh`가 메시지 추출 실패와 무관하게 done-file을 **무조건** 기록(빈 `last_assistant_message` 시 `chmod`+`set -e`로 done이 누락되던 잠재 결함 차단). **검증(TRUSTED e2e, 2026-06-11)**: 동일 시나리오에서 `SESSION_DIR exists=yes`, `claude-done`/`claude-result.json` 모두 기록, provider 완료가 done-file 출현 시각(=Stop 발화 +122s)과 일치, `[completion] monitor timeout … falling back to polling` 로그 부재 → screen-scrape 폴백 미개입. 회귀 테스트 `TestResolveCompletionDetector_HookModeMonitorDisabledStillUsesFileIPC` 추가. |

go 단계 oracle(라우팅 truth-table, env-injection 안전성, hookCollectionEligible, detector 선택, bounded timeout)은 모두 통과한다. **2026-06-11 완료 요약**: session-ready(hook-IPC)는 2026-06-10에, **완료(Stop) done-file IPC 수집은 2026-06-11에 검증 완료**. 마지막 BLOCKING의 근본 원인은 타이밍이 아니라 **완료 detector 선택 결함**이었다 — `resolveCompletionDetector`가 `FileIPCDetector`를 `features.Monitor` 뒤에 게이팅해 모니터 off 시 `ScreenPollDetector`로 떨어졌고, 화면 렌더 응답을 done-file보다 먼저 감지→`Execute` 반환→`defer hookSession.Cleanup()`이 Stop hook 발화 직전 세션 dir을 삭제하는 race를 일으켰다. 수정: HookMode+hookSession이면 monitor flag와 무관하게 FileIPCDetector를 full-budget floor로 우선 선택(`Execute`가 done-file까지 블록 → dir 생존) + `hook-claude-stop.sh` done-file 무조건 기록 하드닝. TRUSTED e2e에서 `SESSION_DIR exists=yes` + `claude-done`/`claude-result.json` 수집 + screen-scrape 폴백 미개입 확인. 따라서 Outcome Lock의 "done-file IPC로 완료 수집되어 0/N timeout 없이 결과 반환"이 충족되어 이 SPEC은 `completed`로 전이한다. Completion evidence (3)(완료 detector=FileIPCDetector 선택)·REQ-004(done-file 감시 완료 수집)가 코드+e2e로 닫혔다.
