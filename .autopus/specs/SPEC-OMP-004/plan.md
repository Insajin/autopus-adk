...
- [x] **T5: OMP native bridge와 artifact lifecycle을 product entrypoint까지 capability-gated로 투영한다.** Generated `/auto go SPEC-ID` route, one-shot `auto pipeline run --platform omp`, sealed five-phase OMP RPC backend, strict receipt/readback와 private runtime을 구현했고 canonical backend의 retry/compaction은 껐다. Local producer(`release-prep.sh --apply` → `auto workflow context-runtime observe-session --explicit-live`)는 pinned candidate/credential을 root-owned sandbox에 격리하고 양 variant를 동일한 10/10 segment로 one-shot UID(`_autopus_v110_*`)에서 순차 실행한다(exec-smoke만 `nobody`). 각 segment의 fresh process/session, sequence reset, unique receipt와 stable provider authority를 검증해 K3-signed orphan evidence로만 승격한다. Release `v0.50.114`(2026-09-03)가 `omp/17.2.7` cohort 42/42, 14/14 gate, evidence tag `omp-context-evidence-v0.50.114`, K3 attestation, static policy 갱신까지 한 transaction으로 완료했다.
...
| T5 active evidence | Deterministic 40-call plan, local root-owned one-shot-UID sandbox, symmetric 10/10 paired-session producer, four unique session receipts, K3 signer, active/historical verifier와 anonymous orphan evidence publication | release v0.50.114: 14/14 gates, evidence tag + K3 attestation + static policy 갱신 완료 |
...
| User active admission | Managed active coordinator가 static policy·K3 attestation·release lineage를 읽어 admission; pin(omp/17.2.7)과 다른 설치본은 fail-closed canonical full | immutable release evidence 확보; pinned 설치본에서 active, 그 외 fail-closed(S8) |
...
|---|---|---|
| (none) | — | release v0.50.114 evidence(2026-09-03)가 active-history authority를 닫았다 |
