...
- **Active wiring boundary**: Production producer는 pinned source의 40-call AB/BA plan과 실제 candidate/provider credential을 root-owned sandbox에 넣고, `gpt-5.6-sol`의 `320000` model window에서 양 variant를 같은 10/10 segment로 실행한다. Segment 분할은 model window가 아니라 20 pair / segment당 10 pair 상수에서 나오지만, full variant는 segment 안에서 history를 압축하지 않고 누적하므로 provider가 실제로 허용하는 요청 크기가 약 `300000` token 이상이어야 하고 managed startup의 `text,image` capability 게이트도 통과해야 한다. 200k hard limit 모델은 pair 5 부근에서 provider가 요청을 거부하므로 이 cohort를 실행할 수 없다. 각 segment는 `nobody`의 fresh full/optimized OMP RPC process/session을 내부에서만 재사용하고, evaluator child의 `--max-time`은 segment 전체 workload(pair당 primary turn + optimized 재사용 call마다 compaction turn)를 덮어야 한다. Report는 2+2 starts, 1-based sequence reset, 네 unique receipt, stable provider authority, zero contamination을 요구한다. K2 서명·active freshness·anonymous orphan readback 전에는 fail-closed다.
