...
- **Active wiring boundary**: Production producer는 pinned source에서 결정론적 40-call AB/BA plan을 만들고 실제 release candidate와 provider credential만 root-owned sandbox에 주입해 `nobody`가 한 RPC session에서 순차 실행하도록 연결됐다. 성공 report는 K2 서명, active freshness, anonymous HTTPS orphan-tag 재검증을 통과해야 release coordinate로 승격된다. 현재 source에는 이 경로의 실제 provider-bound v0.50.98 관측값이 아직 없으므로 active-history authority는 계속 fail-closed다.
