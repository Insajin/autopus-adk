...
- **Active wiring boundary**: Production producer는 pinned source의 40-call AB/BA plan과 실제 candidate/provider credential을 root-owned sandbox에 넣고, `gpt-5.6-sol`의 `320000` model window에서 양 variant를 같은 10/10 segment로 실행한다. 각 segment는 `nobody`의 fresh full/optimized OMP RPC process/session을 내부에서만 재사용한다. Report는 2+2 starts, 1-based sequence reset, 네 unique receipt, stable provider authority, zero contamination을 요구한다. K2 서명·active freshness·anonymous orphan readback 전에는 fail-closed다.
