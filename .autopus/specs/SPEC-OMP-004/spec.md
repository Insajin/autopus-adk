...
- **Active wiring boundary**: Production producer는 pinned source의 40-call AB/BA plan과 실제 candidate/provider credential을 root-owned sandbox에 넣고, image-capable reasoning 모델의 실제 model window에서 양 variant를 같은 10/10 segment로 실행한다. Segment 분할은 모델 window가 아니라 20 pair / segment당 10 pair 상수에서 나오므로 provider·model은 release operator가 고르되 managed startup의 `text,image` capability 게이트를 통과해야 한다. v0.50.98–v0.50.100 authority는 `openai-codex/gpt-5.6-sol`의 `320000` window였고, v0.50.101 authority는 `anthropic/claude-sonnet-4-5`의 `200000` window다. 각 segment는 `nobody`의 fresh full/optimized OMP RPC process/session을 내부에서만 재사용한다. Report는 2+2 starts, 1-based sequence reset, 네 unique receipt, stable provider authority, zero contamination을 요구한다. K2 서명·active freshness·anonymous orphan readback 전에는 fail-closed다.
