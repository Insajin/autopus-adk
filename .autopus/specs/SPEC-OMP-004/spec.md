...
- **Lifecycle state**: T9의 선행 integrated verification, native `/auto` canonical-full pipeline, 명시적 local production evidence producer와 corrective provider-bound cohort까지 관측했지만 K2-signed immutable release evidence와 active-history 승격은 아직 없으므로 `in-progress`다. 코드 경로나 unsigned diagnostic만으로 `implemented` 또는 `completed`로 승격할 수 없다.
...
- **Active wiring boundary**: Production producer는 pinned source의 40-call AB/BA plan과 실제 candidate/provider credential을 root-owned sandbox에 넣고, 제품 기본 tier 모델의 실제 model window에서 양 variant를 같은 10/10 segment로 실행한다. Segment 분할은 model window가 아니라 20 pair / segment당 10 pair 상수에서 나오지만, full variant는 segment 안에서 history를 압축하지 않고 누적하므로 provider가 구독 자격으로 실제 허용하는 단일 요청 크기가 약 `300000` token 이상이어야 하고 managed startup의 `text,image` capability 게이트도 통과해야 한다. 200k hard limit 모델은 pair 5 부근에서 provider가 요청을 거부하므로 이 cohort를 실행할 수 없다. v0.50.98–v0.50.100 authority는 `openai-codex/gpt-5.6-sol`의 `320000` window였고, v0.50.104 authority는 제품 `standard` tier 기본값인 `anthropic/claude-sonnet-5`의 `1000000` window다. 각 segment는 `nobody`의 fresh full/optimized OMP RPC process/session을 내부에서만 재사용하고, evaluator child의 `--max-time`은 segment 전체 workload(pair당 primary turn + optimized 재사용 call마다 compaction turn)를 덮어야 한다. Report는 2+2 starts, 1-based sequence reset, 네 unique receipt, stable provider authority, zero contamination을 요구한다. K2 서명·active freshness·anonymous orphan readback 전에는 fail-closed다.
- **Verification evidence**: 기존 T9 baseline은 installed managed/product live canary와 changed production exact aggregate `85.02%`(`3075/3617`)였다. Canonical-full delta는 installed loopback canary, focused/package race, TRUST/security/annotation, vet/build/strict와 exact aggregate `85.04%`(`3321/3905`)를 통과했다. Corrective candidate `c0906156`의 explicit-live 10/10 diagnostic은 40/40 calls, 2+2 process/session starts, 네 unique receipt, 8 compaction admissions, zero contamination, token reduction `2556`bp와 14/14 gate PASS를 관측했다. K2-signed immutable release-source evidence와 active promotion은 release transaction에서 별도 검증한다.
