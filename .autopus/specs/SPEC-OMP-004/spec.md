...
- **Lifecycle state**: T9의 선행 integrated verification과 native `/auto` canonical-full pipeline, 명시적 local production evidence producer까지 구현됐지만 실제 provider-bound v0.50.98 cohort와 active-history 승격은 아직 관측 전이므로 `in-progress`다. 코드 경로만으로 `implemented` 또는 `completed`로 승격할 수 없다.
...
- **Active wiring boundary**: Production producer는 pinned source의 40-call AB/BA plan과 실제 candidate/provider credential을 root-owned sandbox에 넣고, `gpt-5.6-sol` 272k 경계에 맞춰 양 variant를 같은 9/9/2 segment로 실행한다. 각 segment는 `nobody`의 fresh full/optimized OMP RPC process/session을 내부에서만 재사용한다. Report는 3+3 starts, 1-based sequence reset, 여섯 unique receipt, stable provider authority, zero contamination을 요구한다. K2 서명·active freshness·anonymous orphan readback 전에는 fail-closed이며 실제 v0.50.98 cohort는 아직 없다.
- **Verification evidence**: 기존 T9 baseline은 installed managed/product live canary와 changed production exact aggregate `85.02%`(`3075/3617`)였다. Canonical-full delta는 installed loopback canary, focused/package race, TRUST/security/annotation, vet/build/strict와 exact aggregate `85.04%`(`3321/3905`)를 통과했다. Local production producer와 K2 release transaction은 focused Go·shell·workflow contract에서 검증하며 실제 40-call cohort와 immutable evidence는 release 시 별도 관측한다.
