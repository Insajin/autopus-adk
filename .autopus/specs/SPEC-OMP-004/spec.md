...
- **Lifecycle state**: `completed`. T1~T9의 verification, native `/auto` canonical-full pipeline, local production evidence producer에 더해 release `v0.50.114`(2026-09-03, immutable, GitHub release `382345734`)가 K3-signed immutable release-source evidence를 실었고 active static policy가 그 evidence를 가리킨다. 승격 근거는 아래 Active evidence 항목이며, 코드 경로나 unsigned diagnostic이 아니라 서명된 release evidence다.
- **Implemented evidence**: T1~T4와 T6~T8에 더해 body-free correlated `ui.confirm` pre/post ACK, nonce/binding/options/session hash, authoritative canonical rebuild, private stdio `--no-session`, task-owned `0700/0600` runtime, source identity/extension allowlist/environment uniqueness, cleanup lease, cancelable frame reader, preflight cleanup, managed-driver reuse block와 overlay shadow rollback primitive가 구현됐다. Generated `.omp/extensions/autopus-pipeline.ts`의 shell-free `/auto go SPEC-ID` route는 `auto pipeline run ... --platform omp`를 한 번만 호출하고, run-scoped backend는 다섯 canonical phase를 동일한 isolated OMP RPC process에서 실행한다.
- **Installed evidence**: bounded managed canary가 `omp/17.1.8`에서 `provider_requests=3`, `pre_ack=1`, `post_ack=1`, `native_start=1`, `native_end=1`, `same_pid=true`, `same_session=true`, provider #3 exact canonical/transient body, `cleanup_root_count=0`, `sandbox=true`를 관측했다. 2026-09-05 `omp/18.1.10`에서 같은 세 canary(`AUTOPUS_OMP_CONTEXT_LIVE=1`: managed lifecycle, compaction lifecycle threshold 7000/provider_turns 2/optimized_dispatch 1, product session real overlay provider_requests 5/auth 5/pre·post ack 2/2)를 재실행해 동일 oracle이 PASS했다.
- **Active evidence**: release-prep(`scripts/release-tools/release-prep.sh --apply`)이 pinned `omp/17.2.7`(sha256 `cd2f4754…b71ea0`) 위에서 one-shot UID(`_autopus_v110_*`) root-owned sandbox로 40-call AB/BA cohort를 실행했다(exec-smoke만 `nobody`). 결과는 orphan evidence tag `omp-context-evidence-v0.50.114`(commit `fb69234a`, 2026-09-04 06:32 +0900)의 `omp-context-promotion-report.v1.json`(evidence_id `sha256:d896d629…fab67`, 20 tasks/40 observations, full/optimized process·session 2+2, max_concurrency 1, contamination 0, 14/14 gate: integrity, security, quality, pair-count, balanced-order, provider-authority, reusable-session, multi-compaction-admission, token-reduction, fallback, rollback, cleanup, serial-execution, no-retry, provider `openai-codex`, cohort 42/42)와 `omp-context-promotion-attestation.v2.json`(key `omp-context-promotion-2026-q3-k3`)이다. Active signer는 K3이고 K2는 historical-only다. GitHub `OMP_CONTEXT_STATIC_POLICY_B64`(`autopus.omp_context_promotion_runtime.v3`, source_commit `a6d199fb`, auto 0.50.114, omp/17.2.7, provider openai-codex, K3)와 `OMP_CONTEXT_EVIDENCE_{COMMIT_SHA,REPORT_SHA256,ATTESTATION_SHA256,TREE_SHA,TAG_OBJECT_SHA}`가 2026-09-03T21:32Z에 그 evidence로 갱신됐다. `auto pipeline run ... --platform omp`의 managed active coordinator는 이 policy·attestation·lineage를 읽어 admission하며, 설치 OMP가 pin과 다르면(예: 이 워크스테이션의 `omp/18.1.10`) S8대로 fail-closed해 canonical full로 남는다 — 새 pin의 active는 다음 release evidence에서만 열린다.
- **Verification evidence**: T9 baseline은 installed managed/product live canary와 changed production exact aggregate `85.02%`(`3075/3617`)였고, canonical-full delta는 `85.04%`(`3321/3905`)를 통과했다. Corrective candidate `c0906156`의 explicit-live 10/10 diagnostic(40/40 calls, 2+2 starts, 네 unique receipt, 8 compaction admissions, zero contamination, `2556`bp)은 release-prep cohort로 대체·확정됐다. 2026-09-05 HEAD에서 `go test ./...`, vet/build, strict validation, installed canary 3종을 재실행해 green이다.
...
| product entrypoint | `pkg/adapter/omp/omp_native_pipeline_route.go`, `internal/cli/pipeline_backend_omp*.go`, `internal/cli/workflow_context_runtime_product.go` | native `/auto`를 canonical-full long-lived RPC에 연결; active evidence authority 부재 시 compaction을 계속 차단 |
