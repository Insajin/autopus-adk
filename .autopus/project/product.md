...
| Orchestra Detach | `auto orchestra status/wait/result` | pane 터미널(cmux/tmux) 감지 시 auto-detach 비동기 실행, Job 상태 추적, 결과 수집, stale job GC |
...
| Self-Update | `auto update --self` | CLI 바이너리 자동 업데이트 — ECDSA P-256 게시자 서명으로 체크섬 매니페스트 인증, SHA256 검증, 트랜잭션 교체 |
...
19. **CLI 자동 업데이트**: `auto update --self`로 게시자 서명을 인증한 체크섬 매니페스트만 신뢰하고, 최신 바이너리의 SHA256을 검증한 뒤 트랜잭션 방식으로 교체
