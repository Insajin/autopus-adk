...
- `auto verify`는 UI 변경에서 `.autopus/design/verify/latest.json` visual gate report를 생성합니다.
...
| `--viewport <name>` | `desktop` | Playwright project/viewport 선택 (`desktop`, `mobile`, `tablet` 등 프로젝트 설정 기준) |
...
Visual gate report: .autopus/design/verify/latest.json
