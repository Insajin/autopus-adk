...
```bash
auto orchestra brainstorm "{structured idea}" --strategy debate --providers {providers} --rounds 2 --judge {different_model_family_judge} --context --timeout 300 --no-detach --format json
```
...
- judge는 토론 참가자와 분리하고 다른 모델 계열을 사용합니다.
...
3. 참가자와 분리된 다른 모델 계열의 blind judge
...
judge status/model family, dissent appendix를 확인합니다. 어느 하나라도 없으면 Step 5로
진행하지 않습니다.
