...
Ultra의 Opus는 `max`, Sonnet은 `high`를 사용하고, Balanced의 Opus는 `high`,
Sonnet은 `medium`을 사용합니다. Haiku에서는 effort를 생략합니다. 명시적인
`--effort`, `CLAUDE_CODE_EFFORT_LEVEL`, agent frontmatter 값은 기존 우선순위에
따라 Quality Mode 기본값보다 먼저 적용됩니다.
...
| antigravity-cli | `GEMINI.md`, `.gemini/`, `.agents/plugins/autopus/`, `.agents/hooks.json` |
