...
- Codex: Balanced uses Sol/`xhigh` for strategic and Opus-tier work, Terra with role effort for Sonnet-tier work, and Luna with role effort for Haiku-tier work. Ultra uses Sol/`ultra` for the supervisor and orchestra, and Sol/`max` for managed workers.
...
|------|--------------|--------------------------|
| Ultra | supervisor / orchestra | Sol + `ultra` |
| Ultra | managed worker | Sol + `max` |
| Balanced | supervisor / orchestra / Opus-tier worker | Sol + `xhigh` |
...
Codex custom agent files are loaded when a session starts. To change their persistent quality profile, run `auto quality set <mode>`, then `auto update`, and start a new Codex session. A per-run `--quality` override can change a managed orchestra launch, but it cannot hot-swap agents already loaded by the current session.
