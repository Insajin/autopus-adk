...
| **Balanced** | Uses a shared role matrix: top reasoning roles, lighter implementation, and routine verification. Explicit custom tiers remain supported. |
...
|-----------|----------|-------|
| Planning, review, security, debugging, long work | `fable`: planner, architect, spec-writer, reviewer, security-auditor, debugger, deep-worker | same seven-role `fable` core |
| Implementation and testing | `sonnet`: executor, tester, devops, frontend-specialist, perf-engineer | `opus` |
...
Use debugger or deep-worker for complex diagnosis and long-running work; their standard
balanced profiles already use the top model at max. Do not silently promote or demote
the standard executor's model based on task size.
...
| Balanced | quality-managed supervisor | Astra + `xhigh` |
| Balanced | quality-managed orchestra | Astra + `max` |
| Balanced standard | seven-role planning/review/debugging core | Astra + `max` |
| Balanced standard | implementation and routine roles | Luna + `max` |
| Balanced custom tier | explicit nonstandard tier | Existing tier ladder and declared role effort |
