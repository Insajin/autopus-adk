...
if [ -n "$AUTOPUS_ROUND" ]; then
  RESULT_FILE="${SESSION_DIR}/codex-round${AUTOPUS_ROUND}-result.json"
  DONE_FILE="${SESSION_DIR}/codex-round${AUTOPUS_ROUND}-done"
else
  RESULT_FILE="${SESSION_DIR}/codex-result.json"
  DONE_FILE="${SESSION_DIR}/codex-done"
fi

# Read hook JSON from stdin and extract last_assistant_message via python3.
# Codex stop hook provides the same JSON shape as Claude Code stop hook.
...
if [ -n "$AUTOPUS_ROUND" ]; then
  ...
  READY_FILE="${SESSION_DIR}/codex-round${NEXT_ROUND}-ready"
  INPUT_FILE="${SESSION_DIR}/codex-round${NEXT_ROUND}-input.json"
  ABORT_FILE="${SESSION_DIR}/codex-round${NEXT_ROUND}-abort"
