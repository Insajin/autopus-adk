...
Before deriving UX Intelligence, run or inspect the project-local design source pack when available:
...
Use the pack and docs provider report as metadata-only evidence for source availability, token/component refs, provider docs preflight, screenshot/golden refs, Figma refs, compact fetched Figma node metadata, and Code Connect readiness. Missing pack data is a setup gap, not an instruction to invent a new design system.

If `auto design docs` detects Astryx, verify that new Astryx UI work was preceded by template skeleton and component/token docs lookup (`npx astryx template --list --dense`, `npx astryx template <name> --skeleton --dense`, `npx astryx component <Name> --dense`, `npx astryx docs tokens --dense`). If Astryx is not detected, do not require or add Astryx; inspect local `DESIGN.md`, tokens, and shared components instead.
...
- provider-backed component import paths and props when a design-system provider exists
...
When `verify.capture: no-capture` is set in `autopus.yaml`, this agent runs without pixels: do NOT call `page.screenshot`, `toHaveScreenshot`, or any trace/video capture. Steps 3 and 4 are replaced by the four semantic oracles below, and every one of them is required.

| Oracle | Evidence kind | What it replaces the screenshot with |
|--------|---------------|--------------------------------------|
| DOM geometry | `dom_geometry` | Bounding boxes from `boundingBox()` / `getBoundingClientRect`, with overlap, overflow, and viewport-escape assertions |
| Accessibility tree | `accessibility_tree` | `page.accessibility.snapshot()` or ARIA snapshot, with role and accessible-name assertions on every changed control |
| Keyboard navigation | `keyboard_navigation` | Tab-order traversal with focus-visible assertions on each reachable control |
| State transition | `state_transition` | Observable transitions such as loading->ready, error->recovered, or logout->cleared, each asserting the post-state |

Report the collected set as `oracles_collected:[dom_geometry, accessibility_tree, keyboard_navigation, state_transition]` and record `Capture: no-capture` in the result block.

A UX PASS is forbidden while any of the four is missing. Report `Status: BLOCKED` with `missing_no_capture_oracle:<kind>` naming the first missing kind in the order above, and never report `Verdict: PASS` from an incomplete oracle set — a missing oracle is a collection gap, not a pass with less evidence.
...
- Capture: screenshot / no-capture
- Oracles Collected: [oracles_collected list, or n/a in screenshot mode]
