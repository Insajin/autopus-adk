...
- **Plan intent ledger**: optional inline `Clarification Ledger` and `Question Audit` from direct `auto plan` when `--from-idea` is not used

### Clarification Ledger Handoff

If brainstorm context or plan intent ledger contains `## Clarification Ledger`, parse rows by column header name, not absolute column index. Required headers are `Field`, `Status`, `Source`, `Confidence`, `Decision / Assumption`, `If Wrong`, and `Plan Handoff`.
...
If the Visual Brief contains UX wireframe intent, preserve `wireframe intent: assumed` / `wireframe intent: deferred` labels as assumptions, risks, or reviewer focus until confirmed by Outcome Lock or Must acceptance. A low-fi wireframe is an intent probe and communication aid, not final design.
...
If plan intent ledger contains `## Question Audit`, preserve `question_transport`, `question_count`, and `unresolved_fields` in `research.md`.
If the ledger came from direct `auto plan`, preserve it in `research.md` as `## Plan Intent Ledger`; if it came from a BS file, preserve the brainstorm source ref.

If neither the BS file nor plan intent context contains `## Clarification Ledger`, preserve legacy behavior and mention the plain phrase `Clarification Ledger unavailable` in `research.md` rather than fabricating rows.
...
- **Visual Planning Brief**: workflow/state transition은 Mermaid flowchart, UI/UX는 low-fi wireframe, CLI/API/backend는 sequence/data-flow/command-flow로 설명합니다.
  - UX intent wireframe gate: screens, user journeys, navigation/IA, layout, visual hierarchy, component state, interaction, copy, accessibility, responsive behavior, design-system tokens/primitives, or frontend UI files가 관련되면 low-fi wireframe으로 사용자 의도를 파악한 흔적을 남깁니다.
  - Interactive handoff에서는 사용자가 confirm or adjust 한 wireframe 결정을 source evidence로 요약합니다. `--auto` handoff에서는 `wireframe intent: assumed` / `wireframe intent: deferred`를 확정 요구사항이 아닌 risk, validation task, reviewer focus로 보존합니다.
...
### 3.30. Minimality Decision Matrix

`research.md`에는 반드시 `## Minimality Decision Matrix`를 작성합니다. 이 표는 line-count 절감이 아니라 필요한 만큼만 구현하기 위한 결정 증거입니다.

Required ladder rows:
- `actual need`
- `existing code/helper/pattern`
- `stdlib/native`
- `existing dependency`
- `new dependency or abstraction` / `new dependency or new abstraction`
- `minimum sufficient verification`

새 dependency 또는 새 abstraction을 계획할 때는 위 순서대로 `actual need`, `existing code/helper/pattern`, `stdlib/native`, `existing dependency`, `new dependency or abstraction` / `new dependency or new abstraction` 증거를 기록합니다. 앞선 대안 확인 근거가 없으면 해당 확장은 accepted scope가 아니라 `revise-target` 또는 risk로 남깁니다. 사용자가 명시적으로 확장을 요청한 경우에도 intent, considered alternatives, justification, verification obligation을 보존합니다.

`minimum sufficient verification`은 Outcome Lock을 닫는 가장 작은 검증 세트를 고르되, security, validation, accessibility, data-loss, deterministic-oracle, generated-surface-hygiene gate를 줄이지 않습니다.

### 3.35. Risk-First Integration Probe

`plan.md`에는 반드시 `## Risk-First Integration Probe`를 작성합니다. 이 표는 구현 fan-out 전에 통합 결함을 앞으로 당기기 위한 것이며, 기본 prompt-driven pipeline의 `Phase 1.9: Risk-First Probe Gate`가 소비합니다.

- 가장 위험한 implementation assumption 1-3개를 직접 이름 붙입니다. 정규 예시는 실제 제한 권한으로 composition root를 부팅하는 경로, browser -> BFF -> API 최소 round trip, 그리고 logout / cancel / account switch 같은 in-flight 상태 경계입니다.
- 각 행은 `assumption_id`, `class`, `risk`, `boundary`, `input`, `oracle`, `isolation`, `status`, `reason`, `evidence`를 채웁니다.
- 모든 plan 진술은 `requirement_invariant`, `implementation_assumption`, `verified_fact` 중 하나로 분류합니다. `verified_fact`는 실제 실행으로 확인한 사실에만 사용하고 evidence ref를 함께 남깁니다.
...
|---------------|-------|------|----------|-------|--------|-----------|--------|--------|----------|
| A1 | implementation_assumption | high | [실제 통합 경계] | [최소 입력 또는 fixture ref] | [기대 관측 결과] | [격리 방식] | not-run | probe not executed yet | - |
```
...
|-------------|----------|----------|--------------|
| actual need | [Outcome Lock / request evidence] | proceed / revise-target / risk | [important choice] |
| existing code/helper/pattern | [rg/read evidence] | reuse / not applicable | [reused surface/helper] |
| stdlib/native | [stdlib or native platform evidence] | use / not sufficient | [native choice or reason] |
| existing dependency | [manifest/import evidence] | reuse / not sufficient | [dependency reused or skipped] |
| new dependency or abstraction / new dependency or new abstraction | [justification after previous checks] | accepted / revise-target / risk | [why added or skipped] |
| minimum sufficient verification | [focused commands, oracle, parity, safety gates] | required checks | [verification selected] |
...
12. 4개 파일 저장 후 `auto spec validate {SPEC_DIR} --strict`를 실행합니다. 로컬 source tree 검증 중이면 `go run ./cmd/auto spec validate {SPEC_DIR} --strict`를 사용해도 됩니다.
13. `auto spec validate --strict`가 error를 반환하면 provider review로 넘기지 말고, deterministic finding을 먼저 수정한 뒤 다시 실행합니다. 2회 수정 후에도 실패가 남으면 `## Open Issues`에 남은 preflight 오류와 reason을 기록합니다.
...
- research.md는 `## Minimality Decision Matrix`를 포함하고 `actual need`, `existing code/helper/pattern`, `stdlib/native`, `existing dependency`, `new dependency or abstraction` / `new abstraction`, `minimum sufficient verification` 각 row의 evidence, decision, receipt item을 기록
