...
6. `Q-COMP-05`를 적용해 `Semantic Invariant Inventory`의 모든 row가 requirements, plan tasks, oracle acceptance로 추적되는지 확인합니다.
7. 전체 체크리스트를 최대 2회까지 다시 적용합니다.
8. 2회 재시도 후에도 FAIL이 남으면 `spec.md` 말미에 `## Open Issues` 섹션을 추가하고 `Q-* | category | scope | attempt | reason` 형식으로 기록합니다.
