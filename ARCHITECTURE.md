...
| Orchestra | `pkg/orchestra` | Multi-model orchestration (consensus, pipeline, debate, fastest) with judge-based debate, structured consensus, graceful degradation, detach mode (async job execution on pane terminals), and reliability receipts/failure bundles for pane/hook collection paths |
...
| SelfUpdate | `pkg/selfupdate` | CLI binary self-update: GitHub Releases check, ECDSA publisher-envelope verification, SHA256 verification, atomic replace |
...
| Adaptive Quality | `.claude/skills/autopus/adaptive-quality.md` | Task complexity-based execution profiles (HIGH→opus, MEDIUM/LOW→sonnet; Codex uses quality-aware Sol/Terra/Luna profiles; OpenCode keeps its configured model) in Balanced mode |
...
| Self-Update | `pkg/selfupdate/` + `internal/cli/update.go` | GitHub Releases API check, fail-closed ECDSA publisher-envelope verification before SHA256 checksum verification, transactional binary replacement |
