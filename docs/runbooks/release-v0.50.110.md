...
2. The operator is on Darwin arm64, authenticated to GitHub as user ID `204883817`, has the repository administration access required by prep, `sudo -n -u nobody /usr/bin/true` succeeds, and `pgrep -u nobody` reports no pre-existing process before the isolated canary.
